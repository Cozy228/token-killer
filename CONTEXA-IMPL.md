...
- **`packages/` — greenfield compiler.** `core` is a pure library (no long-lived processes; never
  ...
  version is `0.0.0` — **unreleased** `[code: packages/cli/src/mcp.ts:34 server info version "0.0.0"]`.
...
**As merged — tables** `[code: C3 §1, 001-init.sql]`: `entities`(id,kind,name,locator,content_hash,
source_rev,attrs,first_seen,last_verified,gen); `claims`(append-only; subject,predicate,object,
carrier,locus,method,**authority CHECK IN observed/derived/inferred/confirmed**,at,gen);
`links`(resolved current view, "Selection reads THIS, never claims"; src,dst,predicate,method,
...
`[code: C3 §7 — 0 matches for fetch/http/net across packages/*/src]`. These are LAW-conformant
...
unresolvedHere→unavailable; `restricted` reserved until DR-05 lands `[code: A1-3/A3-3]`.
...
merged, a published generation is a **single per-source integer**; worktree digest, schema version,
and analysis-policy version participate in neither the dirty-check nor generation identity, and all
worktrees share one shard — so a clean dirty-check can reuse rows built under another worktree/policy
and serve them `fresh` `[code: shard.ts:4-12; store.ts:855-890; code/adapter.ts:168-181]`. Detail:
`docs/codemap/impl/E-freshness-incremental.md:73`. Refit: bind every published generation to the full
D32 tuple; reject/rebuild on any component mismatch.

**Refit owed (DR-10) — `valid_from`/`valid_to` bitemporal** [LAW §3 bitemporal]. Columns are
written (migration 003) but read by nothing in select/serve `[code: C3 §6 — no downstream reader]`.
Wire them during the DR-03/DR-06 refit, or provide an equivalent bitemporal recompute path. A bare
cut would drop LAW §3's bitemporal promise → LAW-side escalation, not a local decision (Appendix-A
batch Q8).

**Refit owed (DR-09, ORPHAN) — dead usage columns.** `served_count` / `last_served` have no writer
`[code: 001-init.sql:67-68]`; research ruled the usage signal OUT. Cut them (closes O-07 as "cut"
if ratified).
...
- Symbol id `sym:<repo-rel-path>#<qualified.name>[~<disambig>]`; span/content-hash are attributes,
  never id (G-9 — rename/arity change → new id; whitespace shift → same id)
  `[code: extract/code/symbol.ts:6,58-61]`. `SymbolKind = function|method|class|const`.
- Tier-1 languages: TS/TSX/JS, Python, Go, Java, Rust, C# via `web-tree-sitter@^0.25.10` + vendored
  `.wasm` `[code: extract/code/languages.ts:13-21]`.
...
**As merged — event log + fold** `[code: C2 §1]`. Every write is an immutable event appended to a
...
**As merged — drift / anchor-sig** `[code: C2 §3]`. `MemoryDriftReason = target-removed|
...
**As merged — remember surfaces + caller split** `[code: C2 §4]`. `RememberInput.surface ∈
...
**As merged — push/pull** `[code: C2 §5]`. `PushConfig{pin,veto,commitMemory}` from
...
**As merged — doctor E8 + git-depth advisory** `[code: C2 §6]`. `checkMemoryOps` (read-only:
reviewQueue, oldest-review age, dangling/orphan sidecars) + `checkGitDepth` shallow-clone advisory
(always ok:true).

**As merged — identity dedup (D1, slice 6)** `[code: C2 §7]`. `identityCandidatePairs` derives
...
**As merged — MCP protocol** `[code: packages/cli/src/mcp.ts, C3 §5]`. stdio, protocol
...
**As merged — envelope / render** `[code: serve/render.ts, C3 §5]`. ONE markdown text block.
...
**As merged — selection / ranking** `[code: select/engine.ts, rank.ts, C3 §5]`. Composite score =
...
scope, rendered tersely. This DTO is defined fresh by THIS consumer — it does NOT inherit the retired
M3 five-struct kernel (DR-15). **Gate factual machine serving until it is serialized** [LAW R6; art.
2/3].
...
byte-verbatim `[code: src/cli.ts:294]`.

**As merged — hosts / tiers.** 4 hosts (claude-code, copilot-cli, vscode, unknown), ladder
Hook>Shim>Injection `[code: src/shim/hostAdapter.ts:88-204; detect.ts]`. PATH shim wraps only
...
- *CONFORMS (wedge mechanics):* fail-open gate, recovery, telemetry, ledger separation
  `[code: A4-2/3/6/7/8/10]`.
- *DOES NOT conform as evidence [LAW R3; art. 3]:* `summarizeBuild` derives a **success verdict from
  ...
  `[code: src/handlers/system/summary.ts:109-133, verdict line :127]`; and normal in-budget summaries
...
"maintained utility, no scope growth"; death condition = superseded by facet tooling
`[code: A4-9]`.
...
Authority: `docs/design/measurement/{MEASUREMENT-DESIGN.md (ratified P32), R1-GOAL-PROMPT.md}`.

**As merged — harness inventory** `[code: tools/measurement/, C6 §1]`. Committed:
mine-tasks / make-sandbox / run-cell / grade-cell / analyze / run-grid / lib + run-cell-codex /
run-grid-codex + HANDOFF/implementation-notes/task-bank. **Uncommitted (working tree):**
run-cell-codex.ts (modified), README.md (+52), and three untracked Codex-protocol scripts
(run-cell-codex-protocol / run-grid-codex-protocol / analyze-codex-protocol).

**As merged — R1 design.** Two arms only (A = without-ctx, B = pull+push with-ctx); no built "arm
C". N=10 tasks × 3 reps × 2 arms = 60 runs; `--max-budget-usd 3`/cell. Four-condition R1→R2 gate:
pass_B≥pass_A on ≥8/10 · median paired uncached Δ>0 · 90% bootstrap CI excludes 0 · total-input not
ballooned. Task bank is actually 11 tasks (→66 cells) — a 60/66 discrepancy between OPEN.md and
HANDOFF.

**Actual execution state (gitignored `.work/`)** `[code: C6 §4]`: the full grid HAS been run locally,
uncommitted — sonnet grid = HOLD (atlas) / INSUFFICIENT_DATA (token-killer, 31 void cells); codex
gpt-5.5 grid = 100% void; a 2-task Codex-protocol pilot ran. **This exceeds the goal prompt's
"maintainer supervises spend" boundary and no committed report records it.**

**Refit owed (DR-25) — finish + commit a real verdict.** R1 self-measurement is sanctioned [LAW art.
8; O-14] and partially executed but results are uncommitted. Finish → commit a real verdict; measured
numbers only.
...
serve-integrity precondition; V0→V3 is the ladder; M3 is retired; M4/M5 are re-scoped.
...
| **M5 → measurement + wedge perf** | RE-SCOPE (DR-25/DR-28) | M3-retired guide-perf items moot | measurement finished/committed; local wedge perf gates |
| **O-14 R1 self-measurement** | KEEP — finish + commit verdict | sanctioned; DR-25 | committed verdict, measured numbers only |
...
**As merged — test posture** `[code: C3 §Test counts; C2]`. Fixture repos generated into temp dirs
...
serve-blocking DRs (04/05/06/07/31/32) each land with a red→green test before any factual machine
serving is enabled.
...
> Embedded verbatim from `DRIFT-REGISTER.md` (revision 3). This appendix carries the
> normativity/staging header, the Register table, the M-plan v2 skeleton, both Gate-A arbitration
...
| DR-25 | route | CONFORMS-incomplete | art. 8; O-14 | C6 (.work grids: sonnet HOLD / codex 100% void / protocol pilot; all uncommitted) | R1 self-measurement sanctioned + partially executed, results uncommitted. Finish → commit a real verdict; measured numbers only. |
...
- **summarizeBuild exit-code defect** (DR-18) — `src/handlers/system/summary.ts:127` emits
...
- **M3 docs frontmatter** (DR-16) — `docs/build/M3-GOAL-PROMPT.md` + `M3-ACCEPTANCE.md` carry no
  `status:`/`review_after:`; on ratified M3 retirement set `status: superseded` + supersede pointer.
...
- **R1 grid run uncommitted** (DR-25) — a full local grid run exists only in gitignored `.work/`
  ...
  report. Finish + commit a real verdict, or record the run's disposition.
...
- **69–71 (repo layout).** 69 (greenfield, no compressor imports) CARRIED (§1). 70 (packages/
  core+cli+**guide**) CARRIED for core+cli; **the `guide` package is SUPERSEDED/RETIRED** (M3, DR-14
  — never built, C4). 71 (core never spawns / MCP shim in cli) CARRIED (§1, §5).
...
- **103–110 (serving surface).** 103 MCP three tools CARRIED (§5). 104 serving rules CARRIED, but the
  ...
  floor) CARRIED. **108 Guide + 109 on-demand Inferred generation = SUPERSEDED/RETIRED** (M3 retired,
  DR-14/DR-15; zero code, C4) — the guide is prohibited as a destination under LAW art. 1; any future
  admin/inspection tool is separate scope+authority. 110 CLI surface CARRIED (as-merged verbs differ:
...
- **112–119 (build route M1–M5).** **SUPERSEDED** by M-plan v2 (§8). 112 route logic (invisible
  ...
  M1 is built. 115 (M2 scope/acceptance) CARRIED as history — M2 is built. **116 (M3) SUPERSEDED —
  RETIRE** (DR-14). 117 (M4) **RE-SCOPED** per DR-23. 118 (M5) **RE-SCOPED** per DR-25/DR-28. 119
...
superseded = the route (→ M-plan v2 §8) and the guide/M3 (→ retired); dropped = `tk` alias, dead
