// Issue #42 — the optional "tk active" routing beacon.
//
...
// Restore TK_HOOK_BEACON after every test so the default-off cases never leak.
const ORIGINAL = process.env.TK_HOOK_BEACON;
afterEach(() => {
  if (ORIGINAL === undefined) delete process.env.TK_HOOK_BEACON;
  else process.env.TK_HOOK_BEACON = ORIGINAL;
});
...
describe("beaconEnabled — opt-in gating", () => {
  ...
  test("'1' / 'true' / 'yes' → on", () => {
    expect(beaconEnabled({ TK_HOOK_BEACON: "1" })).toBe(true);
    expect(beaconEnabled({ TK_HOOK_BEACON: "true" })).toBe(true);
    expect(beaconEnabled({ TK_HOOK_BEACON: "yes" })).toBe(true);
  });
  test("'0' / 'false' / empty → off (explicit negation)", () => {
    expect(beaconEnabled({ TK_HOOK_BEACON: "0" })).toBe(false);
    expect(beaconEnabled({ TK_HOOK_BEACON: "false" })).toBe(false);
    expect(beaconEnabled({ TK_HOOK_BEACON: "" })).toBe(false);
    expect(beaconEnabled({ TK_HOOK_BEACON: "  " })).toBe(false);
  });
});
...
describe("rewriteBeacon — line shape", () => {
  test("off → undefined regardless of command", () => {
    expect(rewriteBeacon("tk git status", {})).toBeUndefined();
  });
  test("on → 'tk active: routed <cmd>'", () => {
    const line = rewriteBeacon("tk git status", { TK_HOOK_BEACON: "1" });
    expect(line).toBe(`${HOOK_BEACON_PREFIX}: routed tk git status`);
  });
  test("on with no command → bare prefix", () => {
    expect(rewriteBeacon(undefined, { TK_HOOK_BEACON: "1" })).toBe(HOOK_BEACON_PREFIX);
    expect(rewriteBeacon("  ", { TK_HOOK_BEACON: "1" })).toBe(HOOK_BEACON_PREFIX);
  });
});
...
describe("copilot hook — beacon on rewrite is opt-in", () => {
  ...
  test("default off → no additionalContext on a rewrite", () => {
    delete process.env.TK_HOOK_BEACON;
    ...
    expect(out?.modifiedArgs).toEqual({ command: "tk git status" });
  });

  test("opt-in → additionalContext beacon, rewrite untouched", () => {
    process.env.TK_HOOK_BEACON = "1";
    const out = wire({ toolName: "bash", toolArgs: JSON.stringify({ command: "git status" }) });
    expect(out?.additionalContext).toBe(`${HOOK_BEACON_PREFIX}: routed tk git status`);
...
describe("claude hook — beacon on rewrite is opt-in", () => {
  ...
  test("default off → updatedInput only, no additionalContext", () => {
    ...
    expect(out?.hookSpecificOutput.updatedInput).toEqual({ command: "tk git status" });
  });
  ...
  test("opt-in → additionalContext beacon, updatedInput untouched", () => {
    ...
    expect(out?.hookSpecificOutput.additionalContext).toBe(
      `${HOOK_BEACON_PREFIX}: routed tk git status`,
    );
    expect(out?.hookSpecificOutput.updatedInput).toEqual({ command: "tk git status" });
  });
});
