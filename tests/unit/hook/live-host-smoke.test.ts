...
//
// protocol-matrix.test.ts drives ctx's protocol pipeline IN-PROCESS — it mocks
...
// actually invokes: the `ctx hook copilot` PROCESS reading wire bytes from stdin,
...
// `CTX_LIVE_HOST_SMOKE=1` and is intentionally NOT part of `pnpm test:ci`. Run it with:
//
//   CTX_LIVE_HOST_SMOKE=1 pnpm exec vitest run tests/unit/hook/live-host-smoke.test.ts
//
...
const ENABLED = process.env.CTX_LIVE_HOST_SMOKE === "1";

const repoRoot = join(dirname(fileURLToPath(import.meta.url)), "../../..");
const cli = join(repoRoot, "src/cli.ts");
const tsxLoader = pathToFileURL(join(repoRoot, "node_modules/tsx/dist/loader.mjs")).href;

// Spawn the REAL `ctx hook copilot` process and pipe `wire` to its stdin, exactly as a
...
describe.skipIf(!ENABLED)("live-host protocol smoke (CTX_LIVE_HOST_SMOKE=1)", () => {
  ...
  test("Copilot CLI powershell wire → real process emits the modifiedArgs rewrite, exit 0", () => {
    ...
    expect(r.status).toBe(0);
    const out = JSON.parse(r.stdout);
    expect(out.modifiedArgs).toEqual({ command: "ctx git status", mode: "sync" });
