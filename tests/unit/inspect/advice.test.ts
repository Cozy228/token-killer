...
describe("buildAdvice — delivery recommendation leads", () => {
  test("vscode with raw compressible commands → recommend the shim (tg init)", () => {
    ...
    expect(findings[0].recommendation).toContain("tg init");
    expect(findings[0].recommendation).not.toContain("--host copilot-cli");
  });

  test("copilot-cli → recommend tg init --host copilot-cli", () => {
  ...
  test("already-tg commands are not counted as raw", () => {
    const scan = scanWith("vscode", [opp({ key: "tg", kind: "shell", compressible: true, count: 20 })]);
    expect(buildAdvice(scan).some((f) => f.type === "delivery")).toBe(false);
  });
});
...
describe("buildAdvice — per-command & governance findings", () => {
  test("shell-noise rewrite advice for frequent raw commands", () => {
    ...
    expect(shell?.recommendation).toContain("tg git status");
  });
...
describe("rendering", () => {
  ...
  test("advice file has the generated marker header", () => {
    ...
    expect(file).toContain("# CLI Corrections (generated by tg inspect)");
  });
