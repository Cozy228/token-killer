...
beforeEach(() => {
  registerAllRules();
  root = mkdtempSync(join(tmpdir(), "tg-ctx-skills-"));
  home = join(root, "home");
  cwd = join(root, "repo");
...
describe("skill_invocation_policy", () => {
  test("side-effect skill recommends disable-model-invocation (user → safe_mechanical)", () => {
    writeUserSkill("deploy", ["---", "name: deploy", "description: Deploy the service", "---", "# Deploy", "Run the deploy and publish the release."].join("\n"));
  ...
  test("background-knowledge skill recommends user-invocable: false", () => {
    writeUserSkill("conventions", ["---", "name: conventions", "description: Background knowledge about our conventions", "---", "# Conventions", "We use tabs and prefer composition."].join("\n"));
  ...
  test("disciplined skill with disable-model-invocation set has no policy finding", () => {
    writeUserSkill("deploy", ["---", "name: deploy", "description: Deploy", "disable-model-invocation: true", "allowed-tools: [Bash]", "---", "# Deploy", "Run the deploy and publish."].join("\n"));
...
describe("skill_entrypoint_bloat", () => {
  test("long entrypoint recommends progressive disclosure", () => {
    const body = ["---", "name: big", "description: A big skill", "---", "# Big", ...Array.from({ length: 520 }, (_, i) => `line ${i}`)].join("\n");
  ...
  test("compact skill entrypoint has no bloat finding", () => {
    writeUserSkill("small", ["---", "name: small", "description: A small reference skill", "user-invocable: false", "allowed-tools: [Read]", "---", "# Small", "Short overview."].join("\n"));
...
describe("adapter labelling", () => {
  test("Claude-only skill metadata never appears on a copilot-adapter finding", () => {
    writeUserSkill("deploy", ["---", "name: deploy", "description: Deploy", "---", "# Deploy", "Run the deploy and publish."].join("\n"));
    const copilotFindings = userFindings().filter((x) => x.adapter === "copilot");
    expect(copilotFindings.some((x) => /disable-model-invocation|user-invocable|allowed-tools/.test(x.recommendation))).toBe(false);
  });
});
