...
beforeEach(() => {
  home = mkdtempSync(path.join(tmpdir(), "ctx-inspect-cli-"));
  ...
  // Sandbox both here so the seed lands exactly where the spawned `ctx inspect` (which
...
describe("ctx inspect — exit codes & output", () => {
  ...
  test("--text prints the markdown report → exit 0", () => {
    ...
    expect(r.status).toBe(0);
    expect(r.stdout).toContain("# Contexa Inspect");
...
describe("ctx inspect --advice / --write-advice (Slice 5)", () => {
  test("--text --advice leads with the delivery recommendation (vscode → shim)", () => {
    ...
    expect(r.stdout).toContain("ctx install");
  });
  ...
  test("--write-advice writes stable-named artifacts under ~/.contexa/advice", () => {
    seedRawCommands("git status", 6);
    const r = runTk(["inspect", "--write-advice"]);
    expect(r.status).toBe(0);
    const adviceDir = path.join(home, ".contexa", "advice");
