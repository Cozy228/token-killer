...
8. **Top design change #2 — an anchor-freshness pass** (rides the M2 code source): join
   ...
   (`target-removed·signature-changed·body-changed·referencer-changed`, `CTX-IMPL.md:285-287`);
   exclude from push + default pull. **Never auto-delete** (reconsolidation, `workstream-D D-3`).
   Designed (`CTX-IMPL.md:259,96-98,540-544`), unbuilt.
...
10. **Keep the popularity signal OUT** `from-code/inferred`. `served_count`/`last_served` columns
    exist but are unwritten (`CTX-IMPL.md:108-109`); §4 prose wants `authority×usage×recency×
...
|---|---|---|---|---|---|---|---|
| 1 | **Memory's job / separability** | Durable project experience facts; a **thin authoring/import layer** over the shared entity/link/claim graph — one `kind` + side table + two writers + two rank primitives. Not a subsystem. *Rejected:* standalone chatbot memory, global personal memory, wiki/skill authoring, transcript store. | `CTX-DESIGN.md:39-52`; memory owns no store/index/ranker of its own (`workstream-A §0,§5`); survey "structured > vector" (C 5.1) | **Adopt** thin-layer; **Translate** bitemporal/provenance; **Decline** subsystem | Add helper APIs around existing claims/links; never a 2nd store. | High | Users repeatedly needing cross-project personal prefs that aren't project facts and can be scoped without bleed. |
...
- Real push byte cap + sentinel wrapper (`block.ts:26-120`); read-only human surface correctly
  specified (`CTX-DESIGN.md:169-187`).
...
- Anchor-freshness (M2-named, currently total absence; `decayBasis` ages from claim timestamp not
  liveness — `CTX-IMPL.md:540-544`, `select/rank.ts:42-52`).
- Paraphrase/cross-origin echo defense (exact-sentinel only — `sentinel.ts:1-15`; E6 pending).
- The M3 read-only guide (data primitives exist, no assembler/UI — `CTX-IMPL.md:428-435`).
...
- Design says push rank = `authority×usage×recency×anchor-freshness`; code = `authority×recency`
  only (`CTX-DESIGN.md:141-148` vs `push/rank.ts:39-45`).
...
- **Freshness.** Structural: target exists, content hash/fingerprint matches, identity unambiguous;
  ...
  `CTX-IMPL.md:96-98,259`). Drift → `stale-suspect` (reason-classed) → down-rank + push-exclude +
...
3. **Minimal anchor-freshness**: file-target-removed now, symbol signature/body-changed when M2 code
   hashes exist; write reason-classed `stale-suspect` + `needs-review` (`CTX-IMPL.md:540-544`, E2).
...
temp `CTX_HOME` (G-7), injected fixed clock, `assertNoEgress` armed, script-generated git +
...
**From-doc (design/spec):** `CTX-DESIGN.md:21-32,39-67,118-149,169-187` (purpose, six content types,
ranking/serving, push, read-only guide); `CTX-IMPL.md:96-98,184-212,259,285-287,291-335,410-435,
