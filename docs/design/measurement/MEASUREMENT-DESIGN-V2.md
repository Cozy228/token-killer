...
- [ ] E0 tooling BUILT @9f38bc2e; **ground truth AUTHORED 2026-07-10** (11 tasks, anti-leak: fix-commit-only; log in task-bank-review.md + implementation-notes-e0-ground-truth.md). **Gates FROZEN by reviewer pre-run: timeout_rate = 0 · drillability = 1.0 · relevance floor atlas 0.5 / token-killer 0.4** (tk lower because single-file fix targets cap achievable precision, not because the instrument is weaker). Remaining: rebuild sandboxes (picks up push-imperative neutralization + E-14 prompt fix) → run gated E0
