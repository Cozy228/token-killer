...
| E-3 | 30 of the 31 tk voids are ALL 5 tk-* tasks × 2 arms × 3 reps, `void_reason: "is_error/stop_reason=stop_sequence"` — the runner/sandbox dies at turn 1 on every tk task. A harness defect, not model behavior. +1 `claude exit 143 / unparseable json`, +1 atlas void. | (measured) row.json sweep |
