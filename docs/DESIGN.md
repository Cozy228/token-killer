...
| Copilot context optimizer | planned | §4；静态 context 作为 `source = static_context` 分析器并入唯一的 `tg inspect`（§9，默认全跑），`tg optimize context` 为下游消费者（读 `inspect/latest.json`，无则触发 inspect）；非独立 `--copilot-context` 扫描命令；`docs/context-optimizer-implementation-goal.md` |
| Managed token budget block | planned | §5 |
| Inspect | shipped | §9；`docs/inspect-v1-design.md`、`docs/layer2-hooks-inspect-goal.md`。Slice 4 只读扫描器：`src/inspect/sources.ts`（VS Code workspaceStorage / copilot-cli 发现，缺失=not-found）、`src/inspect/scan.ts`（复用 hook normalizer，按 output volume 排名 opportunity，sanitized label 不存原文，session inventory≠transcript coverage，--since/--session 过滤）、`src/inspect/report.ts`（Markdown 默认 + `--json`，列 count/share/total+max output+tokens/input/success/failure）、`src/inspect/repoContext.ts`（`--repo-context` 仅 presence）、`src/inspect/cli.ts`（exit 0/1/2/3）；测试 `tests/unit/inspect/*`、`tests/integration/inspect.test.ts`。当前为 runtime 分析器；`source = static_context` 静态 context 分析器为 §4 计划扩展（统一 `Finding` 模型见 §9.0） |
...
静态 context 分析由唯一的 `tg inspect`（§9）默认承担，不再有独立的 `--copilot-context` 扫描命令。可选的窄化 flag 只缩小分析器范围，不改变"一个 inspect"的事实：

```bash
tg inspect                              # 默认全跑：runtime + 静态 context 全部分析器
tg inspect --copilot-context            # 只跑静态 context 分析器（窄化，非独立扫描）
...
- **Static context（静态上下文）**：扫描 instructions / prompt files / custom agents / AGENTS.md/CLAUDE.md / skills / stable prefix 这些 Copilot 自动带入模型的上下文表面（finding 分类见 §4.3）。这是 inspect 的一组分析器，**不是**独立的 `--copilot-context` 扫描命令。
...
tg inspect --copilot-context  # 窄化：只跑静态 context 分析器
...
报告默认持久化到 `~/.token-guard/projects/<fingerprint>/inspect/latest.json`（`tg optimize context` 优先读它）。
...
1. optimize 先查持久化报告 `~/.token-guard/projects/<fingerprint>/inspect/latest.json`；有则读，没有则触发一次 inspect 现算。
