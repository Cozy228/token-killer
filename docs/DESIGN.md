...
> **CLI 现状以 [ADR 0006](adr/0006-cli-consolidation-and-optimize-apply-engine.md) 为准 (2026-06-07)。** 命令面已整合:`tk shim`→`tk init shim`;`tk report` 别名移除(详细报告走 `tk gain report`);`tk agentsmd`→`tk optimize --token-budget-block`;`tk optimize context`→`tk optimize`;`tk telemetry purge` 不再对用户暴露。`--apply-safe`→`--apply`:git 感知(仓库内同时改 project + user 文件),应用前完整披露 + 备份,`--restore` 可回滚;确定性修复(A 类)自动写,自由文本建议(B 类)只打印。下文命令名已按此更新,但个别"safe subset"措辞以 ADR 0006 为准。
...
| Delivery tiers (shim / `tk init`) | **shipped** | **命令压缩主交付层**；PATH shim 覆盖 VS Code 等全宿主；统一 `tk init` 安装；`src/shim/*`（`gate`/`init`/`install`/`hostConfig`/`path` 三层 hook>shim>inject）；见 [ADR 0002](adr/0002-shim-delivery-tier-and-passthrough.md)、`docs/shim-delivery-goal.md` |
...
| Managed token budget block | shipped | §5；marker block 幂等 insert/remove + 每文件 backup（含 manifest）+ git 感知 apply（仓库内连 project 文件一起写，披露后写、`--restore` 可回滚）：`src/context/applySafe.ts`（`tk optimize --token-budget-block` 安装、`--token-budget-block --restore` 移除）；测试 `tests/unit/context/applySafe.test.ts`、`tests/integration/optimize.test.ts` |
| VS Code token-lean settings | shipped | §4.5；host-native 交付通道：`tk optimize --vscode-settings`（report）/`--apply`（仅 `chat.tools.compressOutput.enabled: true`，全文件 backup + state sidecar 可恢复）/`--restore`；§14/§15 context-surface 与 agent-budget 设置只 advisory、绝不自动改。`src/context/vscodeSettings.ts`，复用 `src/shim/hostConfig.ts` 的 settings I/O；测试 `tests/unit/context/vscodeSettings.test.ts` |
...
**没有独立的 `tk hook install`/`tk hook init`/`tk hook status` 子命令**：安装是 `tk init` 的职责，运行时处理是 `tk hook copilot` 的职责。没有 `tk hook posttool`（`modifiedResult` 结果压缩不做，见[《明确不做》](#明确不做无限期-deferred非-roadmap)）。
...
tk optimize                     # TTY 下进入交互式 optimize；非 TTY 要求显式模式
tk optimize --dry-run           # 读 inspect finding，生成可审查 diff，不写盘
tk optimize --apply        # 只应用低风险机械修复
tk optimize --yes               # 非交互接受全部 direct_restorable；不应用 advice/suggested diff
tk optimize --restore           # 恢复 Token Killer direct modifications
tk optimize --vscode-settings              # 报告 compressOutput 状态 + advisory（只读）
tk optimize --vscode-settings --apply # 仅启用 chat.tools.compressOutput.enabled（可恢复）
tk optimize --vscode-settings --restore    # 还原 compressOutput 到 apply 前的状态
tk optimize --write-advice      # 写入用户级 advice artifact
```
...
tk skill optimize --dry-run  # alias: tk optimize --surface skills --dry-run
tk optimize --token-budget-block            # alias: tk optimize --apply --token-budget-block
tk optimize --token-budget-block --restore          # restore Token Killer managed marker block
```
...
```bash
tk optimize --token-budget-block          # 追加用户级 token budget 规则
tk optimize --token-budget-block --restore        # 移除 tk 追加的用户级内容
```
...
```bash
tk gain report              # 当前项目 fingerprint 的报告（文本格式）
tk gain report --user       # 用户级聚合报告
tk gain report --json       # JSON 格式（机器可读）
tk gain report --csv        # CSV 格式
```
