...
> 面向实现 Contexa 的工程师和 AI Agent。记录产品边界、模块设计和模型治理策略。实现时优先保持简单、可恢复、低侵入，不要把实验能力写成默认承诺。

> **CLI 现状以 [ADR 0006](adr/0006-cli-consolidation-and-optimize-apply-engine.md) 为准 (2026-06-07)。** 命令面已整合:`ctx shim`→`ctx install`;`ctx report` 别名移除(详细报告走 `ctx gain`,默认 HTML);`ctx agentsmd`→`ctx optimize --apply`;`ctx optimize context`→`ctx optimize`;`ctx gain report` / `--token-budget-block` / `--copilot-context` 等旧子命令已删除;`ctx telemetry purge` 不再对用户暴露。`--apply-safe`→`--apply`:git 感知(仓库内同时改 project + user 文件),应用前完整披露 + 备份,`--restore` 可回滚;确定性修复(A 类)自动写,自由文本建议(B 类)只打印。下文命令名已按此更新,但个别"safe subset"措辞以 ADR 0006 为准。
...
企业 agent 工作流不能再默认"多给上下文、多跑命令、多输出文本"。Contexa 要解决无意识 token 膨胀：
...
Contexa 是 Copilot cost-control companion，不是 Copilot wrapper。用户主路径保持不变：
...
- **Command proxy** — 低输出命令 wrappers，RTK 风格改写与压缩。用户通过 `ctx <command>` 前缀使用，是产品的主入口。
- **`ctx config init`** — 用户级配置初始化，生成 Contexa JSONC 配置，不隐式修改项目仓库。
...
`ctx` 的配置和默认写入只支持用户级作用域。项目仓库默认不被写入配置、rules、hooks 或 filters；history 与 raw output 写入 `~/.contexa/projects/<fingerprint>/`（按工作目录 fingerprint 分组）。需要对项目文件给出建议时，`ctx` 输出可审查的建议或 diff，由用户自行决定是否应用。

### Implementation status

| Capability | Status | Code / notes |
|------------|--------|----------------|
| Command proxy (`ctx <command>`) | **shipped** | `src/cli.ts`, `src/handlers/*`, `src/core/pipeline.ts`；覆盖 common / git / js / python / java / cloud / system 七大类（handler 表见 §1.4） |
| Report & history | **shipped** | `src/core/history.ts`, `src/core/report.ts`, `src/core/dataDir.ts` |
| `ctx config init` | **shipped** | `src/core/config.ts`, `src/core/configCli.ts`（closed-set JSONC 模板，仅用户级，`ctx config <init\|show\|path>`） |
| Delivery tiers (shim / `ctx install`) | **shipped** | **命令压缩主交付层**；PATH shim 覆盖 VS Code 等全宿主；统一 `ctx install` 安装（`ctx init` 已重命名）；`src/shim/*`（`gate`/`init`/`install`/`hostConfig`/`path` 三层 hook>shim>inject）；见 [ADR 0002](adr/0002-shim-delivery-tier-and-passthrough.md)、`docs/shim-delivery-goal.md` |
| Hook system | shipped (Copilot-CLI-only, command-rewrite, no `modifiedResult`) | §3；**仅 Copilot CLI**，RTK 式命令改写（无 `modifiedResult`）；rewrite 规则见 §3.8；落地计划见 `docs/layer2-hooks-inspect-goal.md`。Slice 0 normalizer：`src/hook/normalize.ts`（双 dialect、canonical `category`、fail-open）。Slice 1 `ctx hook copilot` 处理器：`src/hook/copilot.ts` 分流、`src/hook/rewrite.ts` 命令改写注册表（只加 `ctx` 前缀）、`src/hook/govern.ts` direct-tool 治理（deny 依赖目录/lockfile、warn 全仓搜索）、`ctx hook check` dry-run（`src/hook/cli.ts`）。Slice 2 prompt/error 事件：`src/hook/prompt.ts`、`src/hook/error.ts`、`history.recordHookFailure` + `source_adapter` 字段。Slice 3 安装：`ctx install --host copilot-cli` 调用 `src/hook/install.ts` 写用户级 `~/.copilot/hooks/ctx-rewrite.json`（指向 `ctx hook copilot`，marker 可恢复，repo 仅 `--project`），并支持 `--dry-run`/`--uninstall`/`--show`；测试 `tests/unit/hook/*`、`tests/unit/shim/initCli.test.ts`、`tests/integration/hook.test.ts` |
| Copilot context optimizer | shipped | §4；静态 context 作为 `source = static_context` 分析器并入唯一的 `ctx inspect`（§9，默认全跑），`ctx optimize` 为下游消费者（读 `inspect/latest.json`，无则触发 inspect）；scope-aware（[ADR 0003](adr/0003-inspect-default-full-static-context.md)：user 默认、`--project` opt-in）；非独立 `--copilot-context` 扫描命令。代码：`src/context/`（`analyzer.ts`、`discover.ts`、`parseMarkdown.ts`、`metrics.ts`、`report.ts`、`patchPlan.ts`、`advice.ts`、`optimizeCli.ts`、`rules/*` 全 §4.3 taxonomy）、`src/inspect/{staticContext,unified}.ts`；测试 `tests/unit/context/*`、`tests/integration/{inspectContext,optimize}.test.ts`；`docs/context-optimizer-implementation-goal.md` |
| Managed token budget block | shipped | §5；marker block 幂等 insert/remove + 每文件 backup（含 manifest）+ git 感知 apply（仓库内连 project 文件一起写，披露后写、`--restore` 可回滚）：`src/context/applySafe.ts`（`ctx optimize --apply` 安装、`ctx optimize --restore` 移除）；测试 `tests/unit/context/applySafe.test.ts`、`tests/integration/optimize.test.ts` |
| VS Code token-lean settings | shipped | §4.5；host-native 交付通道：VS Code `chat.tools.compressOutput.enabled: true` 现已并入 `ctx optimize --apply`（全文件 backup + state sidecar 可恢复）/`--restore`（不再有独立的 `--vscode-settings` flag）；§14/§15 context-surface 与 agent-budget 设置只 advisory、绝不自动改。`src/context/vscodeSettings.ts`，复用 `src/shim/hostConfig.ts` 的 settings I/O；测试 `tests/unit/context/vscodeSettings.test.ts` |
...
| Advice generation | shipped | §10；Slice 5：`src/inspect/advice.ts`（delivery 建议领先：vscode→shim/`ctx install`、copilot-cli→`ctx install --host copilot-cli`；shell-noise rewrite、tool-noise 治理、long-output hotspot；min-confidence/min-occurrences 阈值）、`src/inspect/telemetry.ts`（仅 allow-list 聚合字段）、`src/inspect/persist.ts`（`~/.contexa/advice/` 稳定文件名）；flags `--advice`/`--write-advice`（telemetry 现由 `ctx telemetry` 同意管理，inspect 不再有 `--telemetry-export` flag） |
...
### 明确不做（无限期 deferred，非 roadmap）

以下能力曾作为"后续层"写进设计。经评审判定 ROI 不足，**从设计正文移除、收纳到这里**，以免再次让"实验能力当默认承诺"造成文档漂移（见 [PRINCIPLES.md](./PRINCIPLES.md)）。除非真实使用数据暴露需求，否则不实现。

- **posttool `modifiedResult` 结果压缩** —— 对终端命令冗余：RTK 式 pretool 把命令改写成 `ctx <command>` 后，压缩已由 proxy 完成，无需 posttool 替换结果。唯一增量是压缩 Copilot CLI direct tool 的输出，而 VS Code（主宿主）做不到，收益面极窄。
- **direct tool 结果投影**（`read_file` / `grep_search` / `fetch_webpage` 输出压缩）—— 同样只在 hook 能触发的 Copilot CLI 生效；最糟的膨胀（读 `node_modules`、lockfile、构建产物）已被 pretool governance 的 deny 挡住，边际收益低。
- **Layer 3：model input cacheability 诊断**（stable/volatile prefix、cacheable prefix ratio、prefix churn）—— `ctx` 不控制 Copilot 的 prompt 拼装，只能建议；无法度量，设计自身也声明"不作为 V1 billing claim"，零杠杆。
- **`compact` / `lossless-pointer` 压缩级别** —— 与护城河冲突：`lossless-pointer` 等于"可恢复版 `+N more`"，直接撞 PRINCIPLES 禁止的"假完整"输出。`safe` 是唯一的默认压缩级别。

### Compression Plane Overview

Contexa 的架构可以看成一个 compression plane：不同来源的 evidence 先进入 source-specific adapter，经 output compressor 压缩，最后由 delivery policy 决定 agent 能看到什么。强合同是 command proxy 和 quality gate；direct tool 只做治理（不投影结果），prompt cache 优化（input cache optimizer）已判定不做，见[《明确不做》](#明确不做无限期-deferred非-roadmap)。下图中 `Input cache optimizer` 仅为概念占位，无对应实现。
...
| Source family | Contexa surface | Current / future role |
|---------------|---------------------|------------------------|
| Shell commands | `ctx <command>` | V1 主路径；RTK-style command-aware filtering |
| Terminal tool events | `ctx hook copilot` (preToolUse) | shipped；rewrite terminal commands to `ctx <cmd>`（Copilot CLI；**ADR 0012 起 VS Code 增加 additive hook tier 与 shim 并存,见 #22**） |
| Direct tool events | `ctx hook copilot` (preToolUse) | policy/governance only, not shell rewrite; result projection 不做（见《明确不做》） |
| Prompt / context | `ctx hook copilot` (userPromptSubmitted) | prompt token 阈值 warn/block；input cacheability 诊断不做（见《明确不做》） |
...
Command proxy 是 Contexa 的核心产品能力，用户通过 `ctx <command>` 前缀使用。设计思想来自 RTK：拦截高浪费命令，用专门的 handler 压缩输出。

### 1.1 使用模型

```bash
ctx <original command> [...args]
```

`ctx` 执行原始命令，捕获 stdout/stderr/exit code，通过 handler 压缩输出，记录 token 节省量，并以原始 exit code 退出。
...
```bash
ctx --raw <command...>        # 打印原始输出
ctx --stats <command...>      # 打印 token 节省统计
ctx --verbose <command...>    # 打印统计和 raw output 路径
ctx --max-lines 200 <command...>  # 待实现
ctx --max-chars 12000 <command...> # 待实现
ctx --save-raw <command...>   # 强制保存原始输出
ctx --no-save-raw <command...>
ctx gain [--text|--json|--csv]   # 查看节省报告（默认 HTML，--text 走终端）
ctx --help
ctx --version
```
...
| Data dir | `src/core/dataDir.ts` | `~/.contexa/` 根目录、按 cwd 的 `projectFingerprint`、`history` / `raw` 路径 |
| History | `src/core/history.ts` | JSONL 追加写入 `~/.contexa/projects/<fingerprint>/history.jsonl` |
| Raw store | `src/core/rawStore.ts` | 条件保存原始输出到 `~/.contexa/projects/<fingerprint>/raw/`（exit code ≠ 0 或 >20K chars 自动保存） |
...
只有 **`ctx read --level aggressive`** 属于显式 opt-in 的激进摘要（符号列表）。`cat`、`type`、`less` 以及未指定 level 的 `read` 对大文件也 passthrough 全文。

#### `read --level`（仅 `ctx read`）
...
| Cloud | `curl` | 始终 `-s`；非 JSON body 超 `MAX_RESPONSE_SIZE` 截断,完整 body 经 `ctx --raw` 恢复 |
...
| IaC | `terraform`（`tofu`） | ctx-only（RTK 无对应）:`plan` 剥离 state lock/refresh/data-source read 进度、符号 legend 与 `-out` note,保留完整 resource action body 与 `Plan:` 摘要；`test` 剥离 per-run 进度与 box 边框,保留 failed run、error 诊断与 `Success!/Failure!` 摘要；其他子命令 passthrough |
...
- `safe` 是唯一的压缩级别；`compact` / `lossless-pointer` 不实现，见[《明确不做》](#明确不做无限期-deferred非-roadmap)。

Terminal command 改写规则见 [§3.8 Hook Rewrite Engine](#38-hook-rewrite-engine)（planned）。

### 1.8 Inline failure-fix hints

> **Status: shipped** — `src/core/failureHints.ts`，接入 `cli.ts` 的 `runCompress`。

usage-based billing 下最贵的单位是 round-trip：一个 agent 当下读不懂的失败命令，要再花一个 model turn（外加整段上下文重发）去诊断。当 `ctx` 包裹的命令以**确定性、可模式匹配**的错误失败时，追加一行修复建议，让 agent 不必再发一轮诊断。

硬约束（错误的提示比没有更糟——会把 agent 引向错误路径=更多 round-trip）：

- 只在 `exitCode !== 0` 时触发。
- 只匹配稳定、无歧义的错误串 / 退出码（git push non-fast-forward、unmerged paths、not a git repository、missing package.json script、exit 127 command-not-found、permission denied）。**绝不猜**；无匹配则不追加。
- 提示是 `ctx` 追加的引导，由 presentation 层（`runCompress`）打印在压缩输出**之后**，**不属于压缩输出本身**，不计入 savings、不经过 quality gate，因此永不触发 inflation/omission 回退。
- 仅作用于压缩路径（specific handler + 非 TTY + 非交互）；passthrough 与 `--raw` 不注入。
...
> **Status: shipped（Copilot-CLI-only，命令改写，无 `modifiedResult`）** — pretool 改写 + direct-tool 治理 + prompt/error 事件 + `ctx install --host copilot-cli` 安装均已落地（`src/hook/*`）。posttool 成功路径结果压缩（`modifiedResult`）仍 deferred。

> **交付模型（已锁定，见 [ADR 0002](adr/0002-shim-delivery-tier-and-passthrough.md)、CONTEXT.md《Delivery》、`docs/layer2-hook-protocol-spike.md`）：** 命令压缩的**主交付层是 PATH shim**（host-agnostic，覆盖 VS Code 等所有宿主），不是 hook。GitHub Copilot hook 在用户的 VS Code 企业环境里不触发，因此**本节的 hook 只面向 Copilot CLI**，并走 **RTK 式**：pretool 把终端命令改写成 `ctx <command>` 前缀，压缩由 `ctx` proxy 完成，**hook 不做 posttool `modifiedResult` 结果替换**。direct tool 的结果压缩（需 `modifiedResult`）目前 **deferred**；hook 对 direct tool 只做治理（deny 依赖目录/lockfile 读取），不压缩。安装统一走 `ctx install`（默认 `vscode`→shim，`ctx install --host copilot-cli` 装 Copilot CLI hook），**不再有独立的 `ctx hook install`/`ctx hook init` 子命令**。
>
...
Hook 是 Contexa 在 Copilot CLI 工具调用链中的拦截点。它是 `ctx <command>` 之外的运行时适配层：命令代理继续覆盖 shell/terminal 命令，hook runtime 把 Copilot CLI 的 terminal tool event 改写成 `ctx <command>` 前缀。direct tools（`read_file`、`grep_search`、`list_dir`、`file_search`、`fetch_webpage`、GitHub MCP 等）作为一等**治理**对象（deny/warn），但结果压缩 deferred；VS Code 下这些工作交给 shim 与 inspect，不由 hook 处理。
...
| Tool kind | 示例 | pretool 行为（唯一实现的阶段） |
|-----------|------|--------------|
| Terminal command | `powershell`、`run_in_terminal`、`shell` | 提取 command string，安全时 rewrite/suggest 到 `ctx <command>`（压缩由 proxy 完成） |
| Direct read | `read_file`、`view` | 检查路径、大小、目录，阻断依赖目录、构建产物、lockfile 等高成本读取 |
| Direct search | `grep_search`、`rg`、`grep` | 检查搜索范围，提示限定路径或忽略生成目录 |
| Direct list | `list_dir`、`glob`、`file_search` | 检查路径和深度，阻断依赖目录和构建产物 |
| Direct web / MCP | `fetch_webpage`、GitHub MCP file/search tools | 检查 URL、repo scope、query 宽度和输出上限 |
| Edit / mutation | `apply_patch`、`edit`、`replace_string_in_file`、`create_file`、mutating shell commands | 通常不 rewrite；可做路径策略、dry-run/confirmation 提示 |
| Unknown | 未识别工具 | fail-open |

Direct tools 不伪装成 shell command，也不走 `ctx cat` / `ctx rg` rewrite。它们通过 pretool policy 适配（治理）；terminal tools 进入 command rewrite registry 并复用现有 command proxy pipeline（pretool 改写为 `ctx <command>` 前缀）。**hook 只实现 pretool（terminal 改写 + direct tool 治理）；posttool `modifiedResult` 结果压缩不做**，见[《明确不做》](#明确不做无限期-deferred非-roadmap)。

Hook 只支持用户级安装。Hook 配置写入用户级 Contexa 数据目录，影响该用户显式接入的 Copilot surface；项目仓库不保存 hook 配置。
...
**配置进宿主的命令是 `ctx hook copilot`**（仿 `rtk hook copilot`）——单一 host 命名的处理器，从 stdin 读 Copilot hook payload，按 event 分流：`preToolUse` 把终端命令改写成 `ctx <command>` 前缀（direct tool 只治理）、`userPromptSubmitted` 做 prompt 检查、`errorOccurred` 追加恢复建议。它**只加前缀、不替换结果**，压缩交给 `ctx` proxy。

```bash
ctx hook copilot     # 写进 ~/.copilot/hooks/ 的事件处理器（RTK 式命令改写，仅 Copilot CLI）
ctx hook check <cmd> # dry-run：查看某条命令会被如何改写（仿 rtk hook check）
```

**安装/卸载统一走 `ctx install` / `ctx uninstall`**（`ctx init` 已重命名为 `ctx install`），它把上面的命令 patch 进宿主 hook 配置——`ctx hook copilot` 本身不是安装器，而是 `ctx install` 指向的运行时处理器：

```bash
ctx install                      # 自动探测 host（vscode → 装 shim，命令压缩主交付层）
ctx install --host copilot-cli   # 把 ctx hook copilot 写进 ~/.copilot/hooks/（仿 rtk init --copilot）
ctx doctor                       # 诊断 host/hook/shim/injection + 度量存储健康（只读；--fix 修复+归一化）
ctx install --dry-run            # 预览改动，不写盘
ctx uninstall                    # 移除 ctx 安装的一切（可选 --purge-data 清除度量数据）
```

**没有独立的 `ctx hook install`/`ctx hook init`/`ctx hook status` 子命令**：安装是 `ctx install` 的职责，运行时处理是 `ctx hook copilot` 的职责。没有 `ctx hook posttool`（`modifiedResult` 结果压缩不做，见[《明确不做》](#明确不做无限期-deferred非-roadmap)）。
...
写入位置（**ctx 与 RTK 的分歧**：RTK 默认写进项目仓库 `.github/`，ctx 默认用户级）：

- **Copilot CLI（默认）→ 用户级 `~/.copilot/hooks/ctx-rewrite.json`**，符合「默认不写项目仓库」（§15）。
- 仓库级 `.github/hooks/ctx-rewrite.json` 仅 `ctx install --project` 显式 opt-in。
- 兜底注入 `.github/copilot-instructions.md`（仿 RTK，告诉模型给命令加 `ctx` 前缀）：默认用户级，`--project` 才写仓库。
...
- 对 terminal tools 中的 `rg`、`grep`、`cat`、`git status`、`npm test` 等给出 `ctx` wrapper 改写。
...
### 3.3 posttool hook —— 不做

成功路径的结果压缩（`modifiedResult`）**不实现**，理由见[《明确不做》](#明确不做无限期-deferred非-roadmap)：RTK 式 pretool 改写后 `ctx` proxy 已自压缩，posttool 替换结果对终端命令冗余。hook 只在 pretool 阶段改写命令 / 治理 direct tool；失败路径见 §3.4。
...
- `rewrite` — 改写后的 `ctx` 命令字符串
- `suggest` — 不改写，附建议文本
- `pass` — 放行（已经是 ctx 命令、或语义不等价场景）
- `deny` — 阻断并附原因
...
| 原始命令 | 改写 |
|---------|------|
| `rg <pattern> <path>` | `ctx rg <pattern> <path>` |
| `grep -r <pattern> <path>` | `ctx grep -r <pattern> <path>` |
| `cat <file>` | `ctx cat <file>` |
| `git status` | `ctx git status` |
| `git diff` | `ctx git diff` |
| `git log` | `ctx git log` |
| `git branch` | `ctx git branch` |
| `npm test` / `pnpm test` | `ctx npm test` |
| `tsc --noEmit` | `ctx tsc --noEmit` |
| `eslint <path>` | `ctx eslint <path>` |
| `find <path> -name <pattern>` | `ctx find <path> -name <pattern>` |
| `ls <path>` | `ctx ls <path>` |
| `mvn test` | `ctx mvn test` |
| `gradle test` | `ctx gradle test` |

**不改写场景**：

- 已经是 `ctx` 命令 → pass（不嵌套改写）。
- heredoc（`<<EOF`）→ pass。
- redirect write（`>`、`>>`）→ pass。
...
**只有一个 `ctx inspect`。** 静态 context 分析不是一个独立的扫描命令，而是 `ctx inspect`（§9）的一组分析器，和 prompt/session/tool/input/output/cache/skill 等运行时分析器并列，**默认全跑**，产出统一的 finding 报告。context optimizer 不是平行的第二个 inspect，而是 inspect 的**下游消费者**：
...
2. **Optimize**（`ctx optimize`）：读 inspect 的 finding 数据，只处理安全子集；其余输出 suggested diff 或 advice artifact，由用户决定。它先检查是否有持久化 inspect 报告（`~/.contexa/projects/<fingerprint>/inspect/latest.json`），有则读、没有则触发一次 inspect 现算。

很多 context 优化不是脚本能直接完成的代码改写，而是团队 workflow 决策：哪些规则应该 repo-wide，哪些应该 path-specific，哪些 task workflow 应沉淀为 prompt file，哪些 custom agent 需要显式选择。Contexa 必须把这些列为 advisory，不默认重写项目仓库。

实施计划见 [`docs/context-optimizer-implementation-goal.md`](context-optimizer-implementation-goal.md)。

### 4.1 命令

静态 context 分析由唯一的 `ctx inspect`（§9）默认承担，不再有独立的 `--copilot-context` 扫描命令。可选的窄化 flag 只缩小分析器范围与作用域，不改变"一个 inspect"的事实。静态 context **按作用域区分**（ADR 0003）：裸 `ctx inspect` 默认跑**用户级**全局 context（token 杠杆最大、可在任何地方跑），`--project` 选当前仓库；runtime 与作用域正交、始终跑：

```bash
ctx inspect                              # 默认：用户级静态 context + runtime
ctx inspect --project                    # 项目静态 context + runtime
ctx inspect --project --user             # 两级 + runtime
ctx inspect --surface skills             # 进一步窄化到某个 context surface
ctx inspect --json                       # 统一 finding 报告（JSON）

ctx optimize                     # 预览：读 inspect finding，生成可审查 diff，不写盘（bare = 预览）
ctx optimize --apply             # 应用低风险机械修复（含启用 VS Code chat.tools.compressOutput，可恢复）
ctx optimize --restore           # 还原最近一次 --apply / --backup 的改动
ctx optimize --backup [paths]    # 编辑前快照指定文件（缺省=全部 in-scope context 文件），供 --restore 回滚
ctx optimize --surface <name>    # 限定 instructions|prompts|agents|skills 之一
ctx optimize --project | --user  # 强制单一 scope（默认 git-aware）
```

兼容短命令可以保留，但语义必须映射到"inspect 分析 → optimize 动作"模型：

```bash
ctx skill scan                # alias: ctx inspect --surface skills
ctx optimize --apply          # 应用确定性修复（含写入 agent 指令文件的 token-budget guidance）
ctx optimize --restore        # 还原 Contexa 写入的内容
```
...
| Layer | 例子 | 写入策略 |
|-------|------|----------|
| **Direct modify / restorable** | 写入 AGENTS / copilot-instructions managed block，要求 agent 主动使用 `ctx <command>`（仿 RTK instruction injection）；写入 Token Budget block；给用户级 skill 加高置信 invocation frontmatter；把用户级 VS Code `chat.tools.compressOutput.enabled` 设为 `true` | `--apply` 可写；必须 backup 或 marker-based restore |
...
Direct modify 不是“语义重写”。它只允许可逆的、低风险的、Contexa 可解释的机械操作：

- **Instruction injection**：在用户级 AGENTS / copilot-instructions 目标内插入 marker block，要求高输出 shell 命令优先使用 `ctx <command>`；项目级只在显式 `--project` 时写，且同样 marker 可恢复。
...
> **Status: shipped** — 实现 `src/context/vscodeSettings.ts`，入口已并入常规 `ctx optimize` / `ctx optimize --apply` / `ctx optimize --restore` 流程（`--vscode-settings` 已移除）。本表是 finding 13/14/15 的精确 key 列表，是 source of truth；代码与此一致。

| Key | 类别 | token-lean 目标 / 触发 |
|-----|------|------------------------|
| `chat.tools.compressOutput.enabled` | **direct / restorable** | 设为 `true`（host-native 终端输出压缩）。唯一可直接应用的设置 |
| `chat.includeReferencedInstructions` | advisory | `true` → 每次请求自动引入被引用的 instruction 文件 |
| `chat.useNestedAgentsMdFiles` | advisory | `true` → 加载子树 nested AGENTS.md |
| `chat.useCustomizationsInParentRepositories` | advisory | `true` → 拉取父仓 customizations |
| `github.copilot.chat.additionalReadAccessFolders` | advisory | 非空 → 额外读取目录扩大上下文面 |
| `chat.mcp.discovery.enabled` | advisory | `true` → 自动发现 MCP server（额外工具面） |
| `github.copilot.chat.codesearch.enabled` | advisory | `true` → 全仓 code-search 上下文 |
| `github.copilot.chat.edits.suggestRelatedFilesFromGitHistory` | advisory | `true` → 从 git history 拉相关文件 |
| `chat.agent.maxRequests` | advisory | `> 15` → 建议降到 8–12（token-control profile） |
| `github.copilot.chat.agent.autoFix` | advisory | `true` → 额外自主工具循环 |

advisory 项**只检测、只报告，绝不自动改**（每条都编码工作流偏好或可能降低复杂任务成功率）；只有 `compressOutput` 走 direct restorable apply，恢复契约见 §4.5。

默认 `ctx optimize` 在 TTY 中可以是交互式流程：按 `Direct and restorable`、`Suggested diffs`、`Advice only` 分组展示 finding，让用户逐项查看 diff、应用、跳过或写 advice。非 TTY 中 bare command 必须失败并要求选择 `--dry-run`、`--apply`、`--write-advice`、`--restore` 或 `--yes`，不能暗中写盘或等待输入。

### 4.5 Safety policy

- `inspect` 永远只读，不修改 instructions、prompts、agents、skills 或项目仓库。
- `optimize --apply` 默认只写用户级文件；项目内文件只有显式 `--project` 且属于 Contexa managed marker block 时才允许写。
...
向用户级 agent 指令文件追加短 token budget 指示和 `ctx <command>` 使用指令。这是 Copilot context optimizer 中可安全自动化的最小子集，而不是完整 context 优化。

项目内 `AGENTS.md`、`CLAUDE.md`、`COPILOT.md` 只做只读分析或输出建议，不由 Contexa 自动 patch。

### 5.1 命令

```bash
ctx optimize --apply             # 追加用户级 token budget 规则（确定性修复的一部分）
ctx optimize --restore           # 移除 ctx 追加的用户级内容
```
...
- When running terminal commands for the agent, use `ctx <original command>` when a `ctx` handler exists; use raw commands only when exact output or interactivity is required.
<!-- contexa:end -->
```
...
- `restore` 只移除 `<!-- contexa:start -->` 到 `<!-- contexa:end -->` 之间的内容，marker 外的修改不受影响。
- 如文件不存在则跳过或提示后创建。
...
Contexa 不提供项目级自定义 filter engine。输出压缩只来自内置 command handlers、tool-level filters 和 quality gate。

### 6.1 设计约束

- 不读取项目仓库里的 `.contexa/filters.yaml` 或类似规则文件。
...
| Level | Status | 行为 |
|-------|--------|------|
| `safe` | 默认（唯一实现） | 结构化但 retention-first；不能完整表达就 raw passthrough |
| `compact` / `lossless-pointer` | 不做 | 更激进的 projection 与护城河冲突，见[《明确不做》](#明确不做无限期-deferred非-roadmap) |
...
每次 `ctx` 命令执行（hook 拦截 planned）都追加写入：

`~/.contexa/projects/<project_fingerprint>/history.jsonl`

`project_fingerprint` 为 `repo:` + SHA-256(`realpath(cwd)`) 前 12 位十六进制（避免 macOS `/var` vs `/private/var` 路径别名导致 fingerprint 不一致）。测试可通过环境变量 `CONTEXA_HOME` 覆盖数据根目录。
...
`raw_output_path` 为相对于 `CONTEXA_HOME`（默认 `~/.contexa`）的路径。
...
| `compression_level` | `safe`（唯一实际写入的级别；`compact` / `lossless-pointer` 见[《明确不做》](#明确不做无限期-deferred非-roadmap)，不实现） |
...
```bash
ctx gain                     # 当前项目 fingerprint 的报告（默认 HTML，浏览器打开）
ctx gain --text              # 文本格式（终端）
ctx gain --user              # 用户级聚合报告
ctx gain --json              # JSON 格式（机器可读）
ctx gain --csv               # CSV 格式
```
...
- `device_hash` — `sha256(deviceSalt)`, a per-install **anonymous** id (ADR 0004).
  ...
  the internal `purgeState()` helper deletes the salt and resets it (no longer a
  user-facing `ctx telemetry` subcommand — ADR 0006). Absent from the local export
...
`ctx inspect` 是 Contexa **唯一的只读分析入口**，**默认跑全部分析器**，产出一份统一的 finding 报告。它同时覆盖两类证据：

- **Runtime（会话证据）**：扫描 Copilot 会话历史和本地工具调用，找出终端命令中可用 `ctx <command>` 替代的遗漏机会，以及 direct tool 中读/搜/列目录/web/MCP 操作本应由 hook runtime 治理的高成本模式；并分析 prompt / session / tool / input / output 维度。
...
`ctx optimize`（§4）是 inspect 的下游消费者，读 inspect 的 `source = static_context` finding 做针对性修改。

```bash
ctx inspect                    # 默认：用户级静态 context + runtime（作用域见 ADR 0003）
ctx inspect --project          # 项目静态 context + runtime
ctx inspect --user             # 仅用户级（与默认同；可与 --project 组合表示两级）
ctx inspect --input-type vscode
ctx inspect --input-type copilot-cli
ctx inspect --surface <name>   # 窄化到某个 context surface（instructions/prompts/agents/skills）
ctx inspect --since 7d         # 仅扫描最近 7 天（只作用于带可靠时间戳的 runtime 记录）
ctx inspect --session <id>     # 扫描指定 session
ctx inspect --json             # JSON 格式输出（统一 finding 报告）
ctx inspect --advice           # 报告附加详细 advice 附录
ctx inspect --write-advice     # 写入用户级建议文件（持久 artifact）
ctx inspect --fail-on <sev>    # opt-in：存在 ≥该 severity 的 finding 时 exit 4（info|warn|error）
```

runtime-only flag（`--since`/`--session`/`--input-type`）只作用于会话证据轴；scope flag
（`--project`/`--user`）与分析器轴自由组合。Exit code：
...
// "delivery" = runtime finding，动作是"装 shim/hook（ctx install）"，不改任何文件
...
报告默认按**作用域分桶**持久化（ADR 0003）：用户级到 `~/.contexa/user-context/inspect/latest.json`，项目级到 `~/.contexa/projects/<fingerprint>/inspect/latest.json`（`ctx optimize` 按作用域优先读对应桶）。每条 finding 带 `scope`（user/project）；runtime finding 与作用域正交。
...
作为 `ctx inspect` 的扩展能力，advice generation 分析 Copilot agent 的重复浪费行为模式，生成 CLI 使用建议、hook 策略建议和用户级规则文件。

### 10.1 命令

```bash
ctx inspect --advice                    # 输出浪费模式报告
ctx inspect --since 14d --advice        # 分析最近 14 天
ctx inspect --write-advice              # 写入用户级 advice 文件
ctx inspect --json --advice             # JSON 格式输出
ctx inspect --min-confidence 0.7        # 最低置信度阈值
ctx inspect --min-occurrences 5         # 最低重复次数阈值
```
...
| Terminal 搜索依赖目录 | terminal command 中 `rg`/`grep` 路径包含 `node_modules/` | rewrite/suggest 到 `ctx rg` 或 deny |
...
| Terminal 执行全量测试 | `npm test` / `pnpm test` 无过滤参数 | rewrite/suggest 到 `ctx npm test` 只看 failures |
...
| 重复执行相同 workflow | 同一 command family 或 direct tool pattern 在短时间窗口内多次出现 | 建议收窄范围、复用报告或使用 ctx；exact command cache 只是辅助建议 |
...
Rule: Prefer ctx rg over raw grep in dependency dirs
  Pattern: grep -r <query> node_modules/
  Occurrences: 12
  Confidence: 0.92
  → Use: ctx rg <query>

Rule: Prefer ctx read --level aggressive for large files
  Pattern: cat <file> where file > 500 lines
  Occurrences: 8
  Confidence: 0.85
  → Use: ctx read --level aggressive <file>
```

### 10.4 自动规则写入

`ctx inspect --write-advice` 将检测到的规则写入用户级 Contexa advice 文件：

```markdown
# CLI Corrections (generated by ctx inspect)

## Prefer ctx rg over raw grep
- **Pattern**: grep -r in dependency directories
- **Correction**: Use `ctx rg` which automatically skips node_modules, dist, and build
- **Detected**: 12 occurrences in the last 14 days

## Prefer ctx read --level aggressive for large files
- **Pattern**: cat on files > 500 lines
- **Correction**: Use `ctx read --level aggressive` for symbol outline; `ctx cat` keeps full file content per retention-first policy
- **Detected**: 8 occurrences in the last 14 days
```

### 10.5 与 Context Optimizer 的关系

运行时分析与静态 context 分析**同属一个 `ctx inspect`**（§9 默认全跑），不是两个互相独立的扫描命令：
...
`ctx optimize` 是 inspect 的**下游消费者**，构成"inspect → optimize"闭环：

1. optimize 先查持久化报告（项目级 `~/.contexa/projects/<fingerprint>/inspect/latest.json`，用户级 `~/.contexa/user-context/inspect/latest.json`）；有则读，没有则触发一次对应作用域的全量 inspect（`ctx inspect --project` 或 `--user`）现算。
...
3. `source = runtime` 的 finding 由 inspect 自身转成投递建议（`fix_class = delivery`：装 shim/hook，即 `ctx install`），不进 optimize 的写盘路径。
...
Contexa 不托管模型路由，但通过策略层级提供治理能力。产品范围停在 L1-L3：建议、行为治理、以及可靠模型名下的策略。
...
实现位置：
- `ctx optimize --apply`：在 agent 指令文件中追加短规则。
- `ctx hook copilot`（`userPromptSubmitted` 事件，仅 Copilot CLI）：对长 prompt 或明显实现型任务追加 `/model` 建议。
- `ctx gain`：按行为类型展示风险分布。
...
实现位置（hook 仅 Copilot CLI，统一入口 `ctx hook copilot` 按事件分流）：
- `preToolUse`：terminal 命令改写为 `ctx` 前缀（由 proxy 压缩）；direct tool 阻断/建议（治理）。
- `userPromptSubmitted`：warn 或 block。
- 无 posttool 阶段（`modifiedResult` 结果压缩不做，见[《明确不做》](#明确不做无限期-deferred非-roadmap)）。
...
| 位置 | 作用域 | 优先级 |
|------|--------|--------|
| macOS / Linux `~/.contexa/config.jsonc` | 用户级 | 唯一配置 |
| Windows `~\.contexa\config.jsonc` | 用户级 | 唯一配置 |
...
- **Model input cache**：provider 对稳定 prompt prefix 的复用。Contexa 不能直接控制它，只能通过稳定布局、确定性格式和 volatile suffix isolation 提高命中机会。
...
- “This command family often benefits from `ctx`.”
...
目标是在工具执行前减少高成本输出。Layer 2 不替代 `ctx <command>`；它把 command proxy **交付**进 Copilot 工具调用链。**主交付层是 shim（host-agnostic，覆盖 VS Code 等全宿主）；hook 是 Copilot-CLI-only 的 RTK 式命令改写**（详见 §3 交付模型与 ADR 0002）。
...
- 新增 rewrite registry，输入 terminal tool 的 raw command，输出 `pass | rewrite | warn | deny`；`rewrite` 只把命令前缀改成 `ctx <command>`，压缩由 `ctx` proxy 完成。
- `ctx hook copilot`（仅 Copilot CLI，配置进 `~/.copilot/hooks/` 的命令）读取 stdin JSON，按事件分流：`preToolUse` 把 terminal command 改写成 `ctx` 前缀（由 proxy 压缩）；direct tools 做路径、范围、大小**治理**（deny/warn），不改写、不压缩。
- `ctx hook copilot` 的 `errorOccurred` 事件在工具失败后追加恢复建议（`additionalContext`），不替换结果，不把失败当成功输出压缩。

> posttool `modifiedResult` 结果压缩与 direct tool 结果投影**不做**，理由见[《明确不做》](#明确不做无限期-deferred非-roadmap)。本层只在 pretool 阶段交付命令改写 + direct tool 治理。
...
- Terminal command：`rg pattern .` → suggest 加路径和 ignore globs，或 rewrite 到 `ctx rg`。
- Terminal command：`git diff` → rewrite 到 `ctx git diff`。
- Terminal command：`npm test`、`pnpm test`、`yarn test` → rewrite 到对应 `ctx` command。
- Direct read：`read_file` / `view` 读取依赖目录、构建产物、lockfile → warn 或 deny（治理）。
- Direct search/list：`grep_search`、`list_dir`、`glob`、`file_search` 访问 repo root、依赖目录或构建产物 → warn/deny（治理）。
- Direct web / MCP：`fetch_webpage`、GitHub MCP file/search tools → pretool 建议收窄 URL、repo、path 或 query（治理）。
- Edit / mutation：`apply_patch`、`edit`、`create_file`、`git commit`、`git stash`、删除/覆盖命令 → 默认不 rewrite；只做长度统计、路径策略和 dry-run/confirmation 建议。

### 13.2 Layer 3: 提高 model input cacheability —— 不做

**不实现**，理由见[《明确不做》](#明确不做无限期-deferred非-roadmap)：`ctx` 不控制 Copilot 的 prompt 拼装，prefix-churn / cacheable-ratio 这类诊断只能给建议、无法度量，且设计自身声明"不作为 V1 billing claim"，杠杆为零。

唯一保留的相邻纪律已落到别处：volatile 内容（timestamp、duration、临时路径、随机文件名）默认不进默认输出、只在 `--verbose` 出现——这由 command proxy 的 delivery policy（§1.7）和 telemetry field policy（§8.3）承担，不需要独立的 Layer 3 子系统。
...
- 默认不写项目仓库；配置、history、raw output、cache、advice 和 backups 都写入用户级 Contexa 目录。
...
├── core/
│   ├── ansi.ts         # ANSI 移除
│   ├── dataDir.ts      # ~/.contexa 路径与 project fingerprint
│   ├── fallback.ts     # 异常兜底
