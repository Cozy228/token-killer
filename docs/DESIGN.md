...
> **CLI 现状以 [ADR 0006](adr/0006-cli-consolidation-and-optimize-apply-engine.md) 为准 (2026-06-07)。** 命令面已整合:`tk shim`→`tk install`;`tk report` 别名移除(详细报告走 `tk gain`,默认 HTML);`tk agentsmd`→`tk optimize --apply`;`tk optimize context`→`tk optimize`;`tk gain report` / `--token-budget-block` / `--copilot-context` 等旧子命令已删除;`tk telemetry purge` 不再对用户暴露。`--apply-safe`→`--apply`:git 感知(仓库内同时改 project + user 文件),应用前完整披露 + 备份,`--restore` 可回滚;确定性修复(A 类)自动写,自由文本建议(B 类)只打印。下文命令名已按此更新,但个别"safe subset"措辞以 ADR 0006 为准。
...
- **Code graph / additive retrieval** — 面向 enterprise VS Code Copilot pilot 的 opt-in graph retrieval；通过 MCP/CLI 暴露 `tk_map`、`tk_read`、`tk_search`、`tk_verify`，减少读/搜/验证 token，但不拦截 Copilot 内置 direct tools。
...
| Code graph / additive retrieval | planned (enterprise pilot) | `src/retrieval/*`；静态 AST/import/PageRank graph + `node:sqlite` graph store，graph-only Node ≥22.13 gate，core 仍 ≥20；CLI under `tk graph`; MCP tools exactly `tk_map` / `tk_read` / `tk_search` / `tk_verify`; explicit install via `tk install --graph`; `tk graph doctor` reports MCP policy / host prerequisites honestly. See [ADR 0013](adr/0013-code-graph-surface-scope.md)、[ADR 0014](adr/0014-tk-becomes-mcp-server.md)、[ADR 0015](adr/0015-node-sqlite-feature-gate.md)、[ADR 0016](adr/0016-measurement-before-feature.md) |
...
- **posttool `modifiedResult` 结果压缩** —— 对终端命令冗余：RTK 式 pretool 把命令改写成 `tk <command>` 后，压缩已由 proxy 完成，无需 posttool 替换结果。唯一增量是压缩 Copilot CLI direct tool 的输出，而 VS Code（主宿主）做不到，收益面极窄。
- **direct tool 结果投影**（`read_file` / `grep_search` / `fetch_webpage` 输出压缩）—— 同样只在 hook 能触发的 Copilot CLI 生效；最糟的膨胀（读 `node_modules`、lockfile、构建产物）已被 pretool governance 的 deny 挡住，边际收益低。**例外不是投影而是 additive retrieval**：Code graph 新增 `tk_map` / `tk_read` / `tk_search` / `tk_verify` 工具供 agent 主动选择；它不修改 host-owned direct tool 输出。
- **Layer 3：model input cacheability 诊断**（stable/volatile prefix、cacheable prefix ratio、prefix churn）—— `tk` 不控制 Copilot 的 prompt 拼装，只能建议；无法度量，设计自身也声明"不作为 V1 billing claim"，零杠杆。
...
Token Killer 的架构可以看成一个 compression plane + additive retrieval plane：不同来源的 evidence
先进入 source-specific adapter，经 output compressor 压缩，最后由 delivery policy 决定 agent 能看到
什么；另一路 graph retrieval 提供新的 MCP/CLI 工具，让 agent 少走 broad search/read。强合同是 command
proxy、quality gate、graph anchor verification；direct tool 只做治理（不投影结果），prompt cache 优化
（input cache optimizer）已判定不做，见[《明确不做》](#明确不做无限期-deferred非-roadmap)。下图中
`Input cache optimizer` 仅为概念占位，无对应实现。
...
Source files ────────────────────────> Graph index ─> Additive retrieval tools ─> Model / agent
...
| Terminal tool events | `tk hook copilot` (preToolUse) | shipped；rewrite terminal commands to `tk <cmd>`（Copilot CLI；**ADR 0012 起 VS Code 增加 additive hook tier 与 shim 并存,见 #22**） |
| Direct tool events | `tk hook copilot` (preToolUse) | policy/governance only, not shell rewrite; result projection 不做（见《明确不做》） |
| Source files / retrieval | `tk graph ...` + graph MCP | planned；additive `tk_map`/`tk_read`/`tk_search`/`tk_verify`，不修改 host built-in direct tool results |
| Prompt / context | `tk hook copilot` (userPromptSubmitted) | prompt token 阈值 warn/block；input cacheability 诊断不做（见《明确不做》） |
...
tk gain [--text|--json|--csv]   # 查看节省报告（默认 HTML，--text 走终端）
tk --help
tk --version
```
...
usage-based billing 下最贵的单位是 round-trip：一个 agent 当下读不懂的失败命令，要再花一个 model turn（外加整段上下文重发）去诊断。当 `tk` 包裹的命令以**确定性、可模式匹配**的错误失败时，追加一行修复建议，让 agent 不必再发一轮诊断。
...
- 提示是 `tk` 追加的引导，由 presentation 层（`runCompress`）打印在压缩输出**之后**，**不属于压缩输出本身**，不计入 savings、不经过 quality gate，因此永不触发 inflation/omission 回退。
...
|-----------|------|--------------|
| Terminal command | `powershell`、`run_in_terminal`、`shell` | 提取 command string，安全时 rewrite/suggest 到 `tk <command>`（压缩由 proxy 完成） |
...
Direct tools 不伪装成 shell command，也不走 `tk cat` / `tk rg` rewrite。它们通过 pretool policy 适配（治理）；terminal tools 进入 command rewrite registry 并复用现有 command proxy pipeline（pretool 改写为 `tk <command>` 前缀）。**hook 只实现 pretool（terminal 改写 + direct tool 治理）；posttool `modifiedResult` 结果压缩不做**，见[《明确不做》](#明确不做无限期-deferred非-roadmap)。

Hook 只支持用户级安装。Hook 配置写入用户级 Token Killer 数据目录，影响该用户显式接入的 Copilot surface；项目仓库不保存 hook 配置。

Direct tool governance strategies:

| Tool family | Governance strategy |
|-------------|---------------------|
| Read / view | Deny/warn for dependency folders, generated output, lockfiles, binary/large-file reads; suggest graph `tk_read` when available |
| Search / grep | Warn on repo-root or generated/dependency search; suggest scoped search or graph `tk_search` |
| List / glob | Warn or deny high-cost dependency/generated traversals |
| Web / GitHub MCP | Check URL/repo/path/query breadth and output-risk hints; never rewrite result content |
| Diagnostics | Measure volume and suggest targeted follow-up; preserve host result semantics |
| Agent / subagent result | Measure output size and detect workflow waste; do not summarize the result |
| Edit / mutation | Measure tool input/output size and surface safety hints; do not rewrite commands or hide patch content |

Actual read/search token reduction for direct-tool-like workflows is handled by the separate
**additive retrieval** surface (`tk_map` / `tk_read` / `tk_search` / `tk_verify`), not by hook
projection.
...
**安装/卸载统一走 `tk install` / `tk uninstall`**（`tk init` 已重命名为 `tk install`），它把上面的命令 patch 进宿主 hook 配置——`tk hook copilot` 本身不是安装器，而是 `tk install` 指向的运行时处理器：
...
**没有独立的 `tk hook install`/`tk hook init`/`tk hook status` 子命令**：安装是 `tk install` 的职责，运行时处理是 `tk hook copilot` 的职责。没有 `tk hook posttool`（`modifiedResult` 结果压缩不做，见[《明确不做》](#明确不做无限期-deferred非-roadmap)）。
...
成功路径的结果压缩（`modifiedResult`）**不实现**，理由见[《明确不做》](#明确不做无限期-deferred非-roadmap)：RTK 式 pretool 改写后 `tk` proxy 已自压缩，posttool 替换结果对终端命令冗余。hook 只在 pretool 阶段改写命令 / 治理 direct tool；失败路径见 §3.4。
...
2. **Optimize**（`tk optimize`）：读 inspect 的 finding 数据，只处理安全子集；其余输出 suggested diff 或 advice artifact，由用户决定。它先检查是否有持久化 inspect 报告（`~/.token-killer/projects/<fingerprint>/inspect/latest.json`），有则读、没有则触发一次 inspect 现算。
...
tk optimize                     # 预览：读 inspect finding，生成可审查 diff，不写盘（bare = 预览）
tk optimize --apply             # 应用低风险机械修复（含启用 VS Code chat.tools.compressOutput，可恢复）
tk optimize --restore           # 还原最近一次 --apply / --backup 的改动
tk optimize --backup [paths]    # 编辑前快照指定文件（缺省=全部 in-scope context 文件），供 --restore 回滚
tk optimize --surface <name>    # 限定 instructions|prompts|agents|skills 之一
tk optimize --project | --user  # 强制单一 scope（默认 git-aware）
```
...
tk optimize --apply          # 应用确定性修复（含写入 agent 指令文件的 token-budget guidance）
tk optimize --restore        # 还原 Token Killer 写入的内容
```
...
| Layer | 例子 | 写入策略 |
|-------|------|----------|
| **Direct modify / restorable** | 写入 AGENTS / copilot-instructions managed block，要求 agent 主动使用 `tk <command>`（仿 RTK instruction injection）；写入 Token Budget block；给用户级 skill 加高置信 invocation frontmatter；把用户级 VS Code `chat.tools.compressOutput.enabled` 设为 `true` | `--apply` 可写；必须 backup 或 marker-based restore |
...
默认 `tk optimize` 在 TTY 中可以是交互式流程：按 `Direct and restorable`、`Suggested diffs`、`Advice only` 分组展示 finding，让用户逐项查看 diff、应用、跳过或写 advice。非 TTY 中 bare command 必须失败并要求选择 `--dry-run`、`--apply`、`--write-advice`、`--restore` 或 `--yes`，不能暗中写盘或等待输入。

### 4.5 Safety policy

- `inspect` 永远只读，不修改 instructions、prompts、agents、skills 或项目仓库。
- `optimize --apply` 默认只写用户级文件；项目内文件只有显式 `--project` 且属于 Token Killer managed marker block 时才允许写。
...
```bash
tk optimize --apply             # 追加用户级 token budget 规则（确定性修复的一部分）
tk optimize --restore           # 移除 tk 追加的用户级内容
```
...
```bash
tk gain                     # 当前项目 fingerprint 的报告（默认 HTML，浏览器打开）
tk gain --text              # 文本格式（终端）
tk gain --user              # 用户级聚合报告
tk gain --json              # JSON 格式（机器可读）
tk gain --csv               # CSV 格式
```
...
- `device_hash` — `sha256(deviceSalt)`, a per-install **anonymous** id (ADR 0004).
  ...
  user-facing `tk telemetry` subcommand — ADR 0006). Absent from the local export
...
`tk optimize`（§4）是 inspect 的下游消费者，读 inspect 的 `source = static_context` finding 做针对性修改。
...
tk inspect --advice           # 报告附加详细 advice 附录
tk inspect --write-advice     # 写入用户级建议文件（持久 artifact）
...
报告默认按**作用域分桶**持久化（ADR 0003）：用户级到 `~/.token-killer/user-context/inspect/latest.json`，项目级到 `~/.token-killer/projects/<fingerprint>/inspect/latest.json`（`tk optimize` 按作用域优先读对应桶）。每条 finding 带 `scope`（user/project）；runtime finding 与作用域正交。
...
`tk optimize` 是 inspect 的**下游消费者**，构成"inspect → optimize"闭环：
...
实现位置：
- `tk optimize --apply`：在 agent 指令文件中追加短规则。
...
### 13.2 Additive retrieval: Code graph

Code graph is the read/search/verify lane that Layer 2 cannot reach through interception. It is
**additive**: tk exposes new tools and guidance, while the host keeps its built-in `read_file` /
`search` tools. The enterprise pilot assumes MCP is enabled by policy; if not, `tk graph doctor`
must say so plainly and the CLI remains the fallback.

v1 surface:

```bash
tk install --graph          # explicit opt-in; installs graph MCP config where supported
tk graph doctor             # Node/MCP/host/index diagnostics, read-only
tk graph index              # explicit prewarm; tools also lazily refresh by file hash
tk graph serve --mcp        # stdio MCP server
tk graph map|read|search|verify
```

MCP tools are capped at four: `tk_map`, `tk_read`, `tk_search`, `tk_verify`. `callers` is a
`tk_search` mode. `light_edit_window` is an explicit `tk_read` mode/flag and returns exact source
slice + anchors + content hash; it is not included in default reads. `tk_verify` compresses local
diff and test-failure evidence; it does not integrate with Copilot's edit loop.

Storage and runtime:

- The graph lives out-of-tree at `~/.token-killer/projects/<fingerprint>/graph.db`.
- The DB may persist paths, symbols, ranges, hashes, ranks, and signature/map snippets, not full
  source read slices.
- Static-only v1: TS/JS/Python/Java using `web-tree-sitter` + `tree-sitter-wasms`; LSP is v2.
- Graph commands require Node ≥22.13 and use graph-only no-warning child re-entry for `node:sqlite`;
  core command compression remains Node ≥20.

### 13.3 Layer 3: 提高 model input cacheability —— 不做

**不实现**，理由见[《明确不做》](#明确不做无限期-deferred非-roadmap)：`tk` 不控制 Copilot 的 prompt 拼装，prefix-churn / cacheable-ratio 这类诊断只能给建议、无法度量，且设计自身声明"不作为 V1 billing claim"，杠杆为零。
...
- Code graph 只在 graph 子命令/MCP 路径要求 Node ≥22.13；core `tk` 仍保持 Node ≥20。
- Code graph index 只能写入 `~/.token-killer/projects/<fingerprint>/graph.db`；不写 in-repo index。
- Graph DB 可保存 signature/map 级片段，但不保存完整源码 read/edit-window slices。
- 模型名不可靠时不猜测，回退 L2。
...
│   ├── stats.ts        # 统计格式化输出
│   └── text.ts         # 文本工具
├── retrieval/           # planned: graph index, MCP tools, targeted read/search/verify
└── handlers/
