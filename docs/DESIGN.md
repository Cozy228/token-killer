...
| Delivery tiers (shim / `tg init`) | planned | **命令压缩主交付层**；PATH shim 覆盖 VS Code 等全宿主；统一 `tg init` 安装；见 [ADR 0002](adr/0002-shim-delivery-tier-and-passthrough.md)、`docs/shim-delivery-goal.md` |
| Hook system | shipped (Copilot-CLI-only, command-rewrite, no `modifiedResult`) | §3；**仅 Copilot CLI**，RTK 式命令改写（无 `modifiedResult`）；rewrite 规则见 §3.8；落地计划见 `docs/layer2-hooks-inspect-goal.md`。Slice 0 normalizer：`src/hook/normalize.ts`（双 dialect、canonical `category`、fail-open）。Slice 1 `tg hook copilot` 处理器：`src/hook/copilot.ts` 分流、`src/hook/rewrite.ts` 命令改写注册表（只加 `tg` 前缀）、`src/hook/govern.ts` direct-tool 治理（deny 依赖目录/lockfile、warn 全仓搜索）、`tg hook check` dry-run（`src/hook/cli.ts`）。Slice 2 prompt/error 事件：`src/hook/prompt.ts`、`src/hook/error.ts`、`history.recordHookFailure` + `source_adapter` 字段。Slice 3 安装：`tg init --host copilot-cli` 调用 `src/hook/install.ts` 写用户级 `~/.copilot/hooks/tg-rewrite.json`（指向 `tg hook copilot`，marker 可恢复，repo 仅 `--project`），并支持 `--dry-run`/`--uninstall`/`--show`；测试 `tests/unit/hook/*`、`tests/unit/shim/initCli.test.ts`、`tests/integration/hook.test.ts` |
| Copilot context optimizer | shipped | §4；静态 context 作为 `source = static_context` 分析器并入唯一的 `tg inspect`（§9，默认全跑），`tg optimize context` 为下游消费者（读 `inspect/latest.json`，无则触发 inspect）；scope-aware（[ADR 0003](adr/0003-inspect-default-full-static-context.md)：user 默认、`--project` opt-in）；非独立 `--copilot-context` 扫描命令。代码：`src/context/`（`analyzer.ts`、`discover.ts`、`parseMarkdown.ts`、`metrics.ts`、`report.ts`、`patchPlan.ts`、`advice.ts`、`optimizeCli.ts`、`rules/*` 全 §4.3 taxonomy）、`src/inspect/{staticContext,unified}.ts`；测试 `tests/unit/context/*`、`tests/integration/{inspectContext,optimize}.test.ts`；`docs/context-optimizer-implementation-goal.md` |
| Managed token budget block | shipped | §5；marker block 幂等 insert/remove + 用户级 backup + 项目级拒写 + 用户级 frontmatter safe-apply（需显式 `--surface`）：`src/context/applySafe.ts`（`tg optimize context --token-budget-block --apply-safe`）、`src/context/agentsmd.ts`（`tg agentsmd patch\|restore`）；测试 `tests/unit/context/applySafe.test.ts`、`tests/integration/optimize.test.ts` |
...
| Advice generation | shipped | §10；Slice 5：`src/inspect/advice.ts`（delivery 建议领先：vscode→shim/`tg init`、copilot-cli→`tg init --host copilot-cli`；shell-noise rewrite、tool-noise 治理、long-output hotspot；min-confidence/min-occurrences 阈值）、`src/inspect/telemetry.ts`（仅 allow-list 聚合字段）、`src/inspect/persist.ts`（`~/.token-guard/advice/` 稳定文件名）；flags `--advice`/`--write-advice`/`--telemetry-export`/`--no-telemetry-export` |
...
| Terminal tool events | `tg hook copilot` (preToolUse) | Future Layer 2；rewrite terminal commands to `tg <cmd>` (Copilot-CLI-only; VS Code uses the shim) |
| Direct tool events | `tg hook copilot` (preToolUse) | Future Layer 2；policy/governance only, not shell rewrite; result projection deferred |
| Prompt / context | `tg hook copilot` (userPromptSubmitted)、Layer 3 diagnostics | Future Layer 3；diagnose stable prefix and volatile suffix behavior |
...
> **Status: shipped（Copilot-CLI-only，命令改写，无 `modifiedResult`）** — pretool 改写 + direct-tool 治理 + prompt/error 事件 + `tg init --host copilot-cli` 安装均已落地（`src/hook/*`）。posttool 成功路径结果压缩（`modifiedResult`）仍 deferred。

> **交付模型（已锁定，见 [ADR 0002](adr/0002-shim-delivery-tier-and-passthrough.md)、CONTEXT.md《Delivery》、`docs/layer2-hook-protocol-spike.md`）：** 命令压缩的**主交付层是 PATH shim**（host-agnostic，覆盖 VS Code 等所有宿主），不是 hook。GitHub Copilot hook 在用户的 VS Code 企业环境里不触发，因此**本节的 hook 只面向 Copilot CLI**，并走 **RTK 式**：pretool 把终端命令改写成 `tg <command>` 前缀，压缩由 `tg` proxy 完成，**hook 不做 posttool `modifiedResult` 结果替换**。direct tool 的结果压缩（需 `modifiedResult`）目前 **deferred**；hook 对 direct tool 只做治理（deny 依赖目录/lockfile 读取），不压缩。安装统一走 `tg init`（默认 `vscode`→shim，`tg init --host copilot-cli` 装 Copilot CLI hook），**不再有独立的 `tg hook install`/`tg hook init` 子命令**。

Hook 是 Token Guard 在 Copilot CLI 工具调用链中的拦截点。它是 `tg <command>` 之外的运行时适配层：命令代理继续覆盖 shell/terminal 命令，hook runtime 把 Copilot CLI 的 terminal tool event 改写成 `tg <command>` 前缀。direct tools（`read_file`、`grep_search`、`list_dir`、`file_search`、`fetch_webpage`、GitHub MCP 等）作为一等**治理**对象（deny/warn），但结果压缩 deferred；VS Code 下这些工作交给 shim 与 inspect，不由 hook 处理。
...
Direct tools 不伪装成 shell command，也不走 `tg cat` / `tg rg` rewrite。它们通过 pretool policy 适配（治理）；terminal tools 进入 command rewrite registry 并复用现有 command proxy pipeline（pretool 改写为 `tg <command>` 前缀）。**当前 goal 范围只实现 pretool（terminal 改写 + direct tool 治理）；上表 posttool 列描述的 `modifiedResult` 结果压缩整体 deferred，是 Copilot CLI 专属的未来能力。**
...
**配置进宿主的命令是 `tg hook copilot`**（仿 `rtk hook copilot`）——单一 host 命名的处理器，从 stdin 读 Copilot hook payload，按 event 分流：`preToolUse` 把终端命令改写成 `tg <command>` 前缀（direct tool 只治理）、`userPromptSubmitted` 做 prompt 检查、`errorOccurred` 追加恢复建议。它**只加前缀、不替换结果**，压缩交给 `tg` proxy。

```bash
tg hook copilot     # 写进 ~/.copilot/hooks/ 的事件处理器（RTK 式命令改写，仅 Copilot CLI）
tg hook check <cmd> # dry-run：查看某条命令会被如何改写（仿 rtk hook check）
```

**安装/卸载统一走 `tg init`**（仿 `rtk init`），它把上面的命令 patch 进宿主 hook 配置——`tg hook copilot` 本身不是安装器，而是 `tg init` 指向的运行时处理器：

```bash
tg init                     # 默认 host=vscode → 装 shim（命令压缩主交付层）
tg init --host copilot-cli  # 把 tg hook copilot 写进 ~/.copilot/hooks/（仿 rtk init --copilot）
tg init --show              # 查看当前 tier / 安装状态
tg init --dry-run           # 预览改动，不写盘
tg init --uninstall         # 移除
```

**没有独立的 `tg hook install`/`tg hook init`/`tg hook status` 子命令**：安装是 `tg init` 的职责，运行时处理是 `tg hook copilot` 的职责。`tg hook posttool`（成功路径 `modifiedResult` 结果压缩）整体 deferred。
...
写入位置（**tg 与 RTK 的分歧**：RTK 默认写进项目仓库 `.github/`，tg 默认用户级）：

- **Copilot CLI（默认）→ 用户级 `~/.copilot/hooks/tg-rewrite.json`**，符合「默认不写项目仓库」（§15）。
- 仓库级 `.github/hooks/tg-rewrite.json` 仅 `tg init --project` 显式 opt-in。
- 兜底注入 `.github/copilot-instructions.md`（仿 RTK，告诉模型给命令加 `tg` 前缀）：默认用户级，`--project` 才写仓库。
...
> **Status: deferred** — 成功路径的结果压缩依赖 `modifiedResult`（Copilot CLI 专属能力），当前 goal 不实现。RTK 式 hook 的压缩由 pretool 改写后的 `tg` proxy 完成，无需 posttool 替换结果；本节描述未来形态。
...
**只有一个 `tg inspect`。** 静态 context 分析不是一个独立的扫描命令，而是 `tg inspect`（§9）的一组分析器，和 prompt/session/tool/input/output/cache/skill 等运行时分析器并列，**默认全跑**，产出统一的 finding 报告。context optimizer 不是平行的第二个 inspect，而是 inspect 的**下游消费者**：
...
2. **Optimize**（`tg optimize context`）：读 inspect 的 finding 数据，只处理安全子集；其余输出 suggested diff 或 advice artifact，由用户决定。它先检查是否有持久化 inspect 报告（`~/.token-guard/projects/<fingerprint>/inspect/latest.json`），有则读、没有则触发一次 inspect 现算。

很多 context 优化不是脚本能直接完成的代码改写，而是团队 workflow 决策：哪些规则应该 repo-wide，哪些应该 path-specific，哪些 task workflow 应沉淀为 prompt file，哪些 custom agent 需要显式选择。Token Guard 必须把这些列为 advisory，不默认重写项目仓库。

实施计划见 [`docs/context-optimizer-implementation-goal.md`](context-optimizer-implementation-goal.md)。

### 4.1 命令

静态 context 分析由唯一的 `tg inspect`（§9）默认承担，不再有独立的 `--copilot-context` 扫描命令。可选的窄化 flag 只缩小分析器范围与作用域，不改变"一个 inspect"的事实。静态 context **按作用域区分**（ADR 0003）：裸 `tg inspect` 默认跑**用户级**全局 context（token 杠杆最大、可在任何地方跑），`--project` 选当前仓库；runtime 与作用域正交、始终跑：

```bash
tg inspect                              # 默认：用户级静态 context + runtime
tg inspect --project                    # 项目静态 context + runtime
tg inspect --project --user             # 两级 + runtime
tg inspect --copilot-context            # 只跑静态 context 分析器（关 runtime，默认用户级）
tg inspect --surface skills             # 进一步窄化到某个 context surface
tg inspect --json                       # 统一 finding 报告（JSON）

tg optimize context --dry-run           # 读 inspect finding，生成可审查 diff，不写盘
tg optimize context --apply-safe        # 只应用低风险机械修复
tg optimize context --write-advice      # 写入用户级 advice artifact
```

兼容短命令可以保留，但语义必须映射到"inspect 分析 → optimize 动作"模型：

```bash
tg skill scan                # alias: tg inspect --surface skills
tg skill optimize --dry-run  # alias: tg optimize context --surface skills --dry-run
tg agentsmd patch            # alias: tg optimize context --apply-safe --token-budget-block
tg agentsmd restore          # restore Token Guard managed marker block
```
...
| Class | 例子 | 写入策略 |
|-------|------|----------|
| Safe mechanical | Token Guard managed marker block、恢复 marker block、用户级文件备份后补明显 frontmatter | `--apply-safe` 可写 |
| Suggested diff | 拆 always-on instruction、前置 review 规则、收窄 `applyTo`、拆 prompt file | `--dry-run` 输出 diff；默认不写 |
| Advisory | instruction 冲突、agent 是否过泛、模型选择是否过贵、workflow 应该归属哪个 surface | `--write-advice` |
| Non-goal | 自动重写团队工作流、删除项目规则、跨 surface 自作主张合并语义 | never apply |

### 4.5 Safety policy

- `inspect` 永远只读，不修改 instructions、prompts、agents、skills 或项目仓库。
- `optimize --apply-safe` 只写用户级文件或 Token Guard 自己管理的 marker block；项目内文件默认只输出 suggested diff。
- 所有写操作先备份，输出 unified diff，并可 restore。
...
向用户级 agent 指令文件追加短 token budget 指示。这是 Copilot context optimizer 中可安全自动化的最小子集，而不是完整 context 优化。
...
`tg inspect` 是 Token Guard **唯一的只读分析入口**，**默认跑全部分析器**，产出一份统一的 finding 报告。它同时覆盖两类证据：

- **Runtime（会话证据）**：扫描 Copilot 会话历史和本地工具调用，找出终端命令中可用 `tg <command>` 替代的遗漏机会，以及 direct tool 中读/搜/列目录/web/MCP 操作本应由 hook runtime 治理的高成本模式；并分析 prompt / session / tool / input / output 维度。
...
`tg optimize context`（§4）是 inspect 的下游消费者，读 inspect 的 `source = static_context` finding 做针对性修改。

```bash
tg inspect                    # 默认：用户级静态 context + runtime（作用域见 ADR 0003）
tg inspect --project          # 项目静态 context + runtime
tg inspect --user             # 仅用户级（与默认同；可与 --project 组合表示两级）
tg inspect --input-type vscode
tg inspect --input-type copilot-cli
tg inspect --copilot-context  # 窄化：只跑静态 context 分析器（关 runtime）
tg inspect --surface <name>   # 窄化到某个 context surface（instructions/prompts/agents/skills）
tg inspect --since 7d         # 仅扫描最近 7 天（只作用于带可靠时间戳的 runtime 记录）
tg inspect --session <id>     # 扫描指定 session
tg inspect --json             # JSON 格式输出（统一 finding 报告）
tg inspect --repo-context     # 附加只读 repo context
tg inspect --write-advice     # 写入用户级建议文件
tg inspect --telemetry-export # 显式导出匿名聚合 telemetry
tg inspect --fail-on <sev>    # opt-in：存在 ≥该 severity 的 finding 时 exit 4（info|warn|error）
```
...
type FixClass = "safe_mechanical" | "suggested_diff" | "advisory" | "delivery" | "non_goal";
// "delivery" = runtime finding，动作是"装 shim/hook（tg init）"，不改任何文件
...
报告默认按**作用域分桶**持久化（ADR 0003）：用户级到 `~/.token-guard/user-context/inspect/latest.json`，项目级到 `~/.token-guard/projects/<fingerprint>/inspect/latest.json`（`tg optimize context` 按作用域优先读对应桶）。每条 finding 带 `scope`（user/project）；runtime finding 与作用域正交。
...
运行时分析与静态 context 分析**同属一个 `tg inspect`**（§9 默认全跑），不是两个互相独立的扫描命令：
...
`tg optimize context` 是 inspect 的**下游消费者**，构成"inspect → optimize"闭环：

1. optimize 先查持久化报告（项目级 `~/.token-guard/projects/<fingerprint>/inspect/latest.json`，用户级 `~/.token-guard/user-context/inspect/latest.json`）；有则读，没有则触发一次对应作用域的全量 inspect（`tg inspect --project` 或 `--user`）现算。
2. 它只取 `source = static_context` 的 finding：`fix_class = safe_mechanical` 的允许 `--apply-safe` 自动写入；`suggested_diff` 默认输出 diff；`advisory` 输出 advice。
3. `source = runtime` 的 finding 由 inspect 自身转成投递建议（`fix_class = delivery`：装 shim/hook，即 `tg init`），不进 optimize 的写盘路径。
...
- `tg hook copilot`（`userPromptSubmitted` 事件，仅 Copilot CLI）：对长 prompt 或明显实现型任务追加 `/model` 建议。
...
实现位置（hook 仅 Copilot CLI，统一入口 `tg hook copilot` 按事件分流）：
- `preToolUse`：terminal 命令改写为 `tg` 前缀（由 proxy 压缩）；direct tool 阻断/建议（治理）。
...
目标是在工具执行前减少高成本输出。Layer 2 不替代 `tg <command>`；它把 command proxy **交付**进 Copilot 工具调用链。**主交付层是 shim（host-agnostic，覆盖 VS Code 等全宿主）；hook 是 Copilot-CLI-only 的 RTK 式命令改写**（详见 §3 交付模型与 ADR 0002）。
...
- 新增 rewrite registry，输入 terminal tool 的 raw command，输出 `pass | rewrite | warn | deny`；`rewrite` 只把命令前缀改成 `tg <command>`，压缩由 `tg` proxy 完成。
- `tg hook copilot`（仅 Copilot CLI，配置进 `~/.copilot/hooks/` 的命令）读取 stdin JSON，按事件分流：`preToolUse` 把 terminal command 改写成 `tg` 前缀（由 proxy 压缩）；direct tools 做路径、范围、大小**治理**（deny/warn），不改写、不压缩。
- posttool 成功路径结果压缩（`modifiedResult`）**deferred**：RTK 式改写后 `tg` 已自压缩，无需 posttool 替换结果。
- `tg hook copilot` 的 `errorOccurred` 事件在工具失败后追加恢复建议（`additionalContext`），不替换结果，不把失败当成功输出压缩。
