...
| Managed token budget block | shipped | §5；marker block 幂等 insert/remove + 用户级 backup + 项目级拒写 + 用户级 frontmatter direct-apply（需显式 `--surface`）：`src/context/applySafe.ts`（`tk optimize context --token-budget-block --apply-safe`）、`src/context/agentsmd.ts`（`tk agentsmd patch\|restore`）；测试 `tests/unit/context/applySafe.test.ts`、`tests/integration/optimize.test.ts` |
| VS Code token-lean settings | shipped | §4.5；host-native 交付通道：`tk optimize context --vscode-settings`（report）/`--apply-safe`（仅 `chat.tools.compressOutput.enabled: true`，全文件 backup + state sidecar 可恢复）/`--restore`；§14/§15 context-surface 与 agent-budget 设置只 advisory、绝不自动改。`src/context/vscodeSettings.ts`，复用 `src/shim/hostConfig.ts` 的 settings I/O；测试 `tests/unit/context/vscodeSettings.test.ts` |
...
tk optimize context --vscode-settings              # 报告 compressOutput 状态 + advisory（只读）
tk optimize context --vscode-settings --apply-safe # 仅启用 chat.tools.compressOutput.enabled（可恢复）
tk optimize context --vscode-settings --restore    # 还原 compressOutput 到 apply 前的状态
...
> **Status: shipped** — 实现 `src/context/vscodeSettings.ts`，入口 `tk optimize context --vscode-settings`（report / `--apply-safe` / `--restore`）。本表是 finding 13/14/15 的精确 key 列表，是 source of truth；代码与此一致。
