...
> _以下「现状」与「改动清单」为规划时点快照（`0fcd6f6` 之前），非当前事实——彼时
> inspect 默认 text、`gain report` 仍在、相关 flag 尚未删除。落地状态见上方 DONE 横幅与下方验收勾选。_
...
### 已决定（续，用户已圈定）
10. **删 `--no-dedup`**（`src/parse.ts:143`）：dedup 默认**开**（与上方现状一致；`TK_SESSION_DEDUP=0`/config 才关），per-command 关它无意义。
