> **SUPERSEDED 2026-07-10** by the root `CONTEXA-DESIGN.md` (design register reconciled to `PRODUCT-DESIGN.md`, the product LAW). This file is the pre-reconciliation register kept as frozen history; claim-by-claim dispositions live in the new register appendix.

# Contexa — Design Document
...
**Contexa is a developer-local context engineering tool**: the project's local context base. Four
functions:
...
you switch tools. Contexa builds the *project's* context, owned by the project: a project fact (why we
...
Contexa cannot, because it is the project's system of record for context. (Consequence for memory storage
+ multi-user sync: `docs/build/MEMORY-DECISIONS.md`, §3/§8 below.)

**Identity**: product name = `contexa`; CLI/MCP command = `ctx` (P20). Core = a TypeScript library + on-disk store;
...
repo files under `.contexa/memory/` (and `.contexa/concepts/`), and the store's `memory`/`concept` rows
...
- **Committed in the repo, git-synced (② authored-local + project presentation):**
  `.contexa/memory/*.md` (append-only entry log), `.contexa/memory/details/<ulid>.md` (multi-line detail
  sidecars, S1), `.contexa/memory/decisions.md` (append-only decision log — lifecycle + resolutions,
  C2/C4), `.contexa/concepts/` (C3), `.contexa/push.jsonc` (pin/veto, shared presentation),
  `.gitattributes` with `merge=union` on the logs (E2).
- **In the repo but gitignored — personal overlay (zone 2):** `.contexa/*.local.*` — my-view
...
```
~/.contexa/projects/<repo-shard>/   # local cache home — outside the repo, never committed
...
- **`nodes`** `(id, kind, name, locator, content_hash, source_revision, attrs, …)`
  ...
  **one table** — cross-source joins and FTS come free. **No `text` payload column (P25①)**:
  ...
  read through to committed `.contexa/memory/` and `.contexa/concepts/` files like every other local
...
- **`memory_meta`** `(node_id, gist, detail?, authority, status, origin, session_ref?,
  ...
  `.contexa/memory/` files: `status` is the deterministic fold over the append-only decision log
...
`store.sqlite` gitignored), **②** authored-local (SoT = committed `.contexa/` files; git + PR; conflicts
...
| `remember()` durable memory · human notes | **②** | git + PR; E1 three-layer conflict | authored-local; SoT = committed `.contexa/memory/` |
| concepts | **②** | git + PR | C3: concepts follow memory into committed `.contexa/concepts/` |
| `.contexa/push.jsonc` (pin/veto) | **②** | git-synced shared config | project presentation (three-tier b); shared, committed |
...
| personal overlay (`.contexa/*.local.*`) | **overlay** | gitignored; never synced | three-tier (c) personal attention + E3 import landing |
...
- **① derived** — never synced; each peer regenerates the store from committed sources (gitignored
  index).
- **② authored-local** (memory, concepts, `push.jsonc`) — committed `.contexa/` files, git + PR. Git
...
- **③ external SoR** — re-imported per person, dated snapshots under `~/.contexa/…/snapshots/`, never
...
committed; (b) **project presentation** (push pin/veto, `.contexa/push.jsonc`) → shared (D27/D30);
(c) **personal attention** (my-view mute/pin) → gitignored personal overlay (`.contexa/*.local.*`),
...
`.contexa/` files; the store becomes a rebuildable, gitignored index; git is the sync/conflict layer
