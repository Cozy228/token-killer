...
Drive agent sessions that give Token Killer a **deterministic, host-agnostic way to put the
...
- **User-level scope only** (CONTEXT.md *User-level*; DESIGN §15). All shim files, PATH
  edits, and `TK_SHIM_DIR` live under `~/.token-killer/` and host user-config
...
signal + interactivity gate), the `tk shim` installer/wrappers, and the unified `tk init`
auto-detect installer with an instruction-injection fallback.

**Not building:** the hook tier installer (`tk hook …`, Track B) — out of scope here;
...
After this phase, `tk <tool>` is shim-safe even with a manually-prepended shim dir:
...
3. **Recursion guard (D2).** New `src/shim/path.ts`:
   ...
   - `executeCommand` **and** `executePassthrough` build `childEnv.PATH =
     stripShimDir(process.env.PATH, process.env.TK_SHIM_DIR)` whenever `TK_SHIM_DIR` is
     set. **Sentinel:** if `resolveReal(program, stripped)` lands inside `TK_SHIM_DIR`
...
- Recursion e2e: build a temp shim dir with a `git` wrapper, set `TK_SHIM_DIR`, run
  `tk git status`, assert it resolves the real git (no recursion, finite).
...
2. **Shimmable-program declaration.** Add an optional `programs?: string[]` to
   ...
   it only on handlers that wrap an external tool; leave it off tk-native verbs
...
3. **Wrapper generation.** `tk init shim install`:
   ...
   - For each shimmable program, write an executable wrapper. POSIX: a file `git`
     ...
     containing `@tk git %*` (and a PowerShell `.ps1` if needed). The wrapper calls `tk`
     by the resolved absolute path of the current `tk` binary (avoid relying on `tk`
     being on PATH inside the shimmed shell).
   - Write a manifest `~/.token-killer/shim/manifest.json` (version, dir, program list,
...
4. **PATH injection (D1), per host.**
   - **VS Code:** patch user `settings.json`
     `terminal.integrated.env.{osx,linux,windows}` to **prepend** `TK_SHIM_DIR` to `PATH`
     and set `TK_SHIM_DIR`. Use `terminal.integrated.env`, **not** shell RC — non-interactive
...
**Phase 2 tests:**
- Wrapper-set derivation excludes tk-native verbs, includes the expected real executables.
...
## Phase 3 — Unified `tk init` (auto-detect ladder) + injection fallback

Independently mergeable on top of Phase 2.

1. **`tk init [--host auto|copilot-cli|vscode] [--global] [--auto-patch] [--show]`** —
...
3. **Hook-tier seam.** If a `tk hook install` (Track B) exists, call it for the hook tier;
...
4. **Instruction injection (lowest tier).** Generate an instruction file telling the model
   to prefix commands with `tk`. **Default target is user-level** (keeps the "never write
   ...
   instruction file under the VS Code user dir; fall back to `~/.token-killer/` + a printed
   note if the host has no user-level slot. **Project-level is explicit opt-in**: only with
   `tk init --project` does it append `.github/copilot-instructions.md` into the repo
...
agent types `git status`
        ...
        ▼
~/.token-killer/shim/git   →   exec tk git status
        │
        ▼
  tk cli main()
        ...
        ▼
  executeCommand(env.PATH = stripShimDir(PATH, TK_SHIM_DIR))   ← real git, no recursion
        ▼
  pipeline → compressed status   (≤ budget, ADR 0001 ladder)

agent types `git commit`  → wrapper → tk → routeSpecific=git-commit, but isInteractive ✓
        → executePassthrough(stdio: inherit)  → real editor opens, tk invisible
```

No cycles: the only loop risk is shim→tk→shim, cut by `stripShimDir` + sentinel.
...
- **Hook tier installer (Track B `tk hook`)** is referenced only as an optional seam in
  Phase 3. If/when Track B ships, `tk init` should prefer it on Copilot CLI. Owner:
