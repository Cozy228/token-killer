...
- `src/parse.ts:143` — `--no-dedup`：session dedup 默认**关**（`TK_SESSION_DEDUP`），per-command 关它几乎无意义。
...
## 验收 → verify
- [ ] `tk inspect` 默认打开 HTML；`tk inspect --text` 出终端文本；`--html` 不再存在（或报 unknown）。
- [ ] `tk gain` 默认打开 HTML；`tk gain --text` 出文本；`tk gain report` 报 unknown subcommand。
- [ ] `tk optimize --dry-run`、`tk <cmd> --report`、`tk gain --format` 均报 unknown flag。
- [ ] `grep -rn "args.dryRun\|--token-budget" src/` 无命中（dryRun 死字段已清）。
- [ ] `--write-advice` 仅在 inspect 生效；optimize 不再接受。
- [ ] `tk <cmd> --no-dedup` 报 unknown flag；`tk --help` 不再出现 `tk shim` 段。
- [ ] `tk optimize --vscode-settings` 报 unknown flag；VS Code 设置问题改由 `inspect` 报 finding、`optimize --apply` 修复。
- [ ] `tk inspect --copilot-context|--repo-context|--telemetry-export` 均报 unknown flag。
- [ ] 全套 `pnpm test` 绿；help 文本同步更新，无悬挂 flag。
- [ ] 手测：`tk inspect`、`tk gain`、`tk gain --text`、`tk optimize --apply` 各跑一遍，输出符合上述。
