...
### 3a — Guide server + shell + Entity Biography (flagship) + evidence drawer
...
- **C6-history**: hot areas + change-coupling (co-change) clusters + recent activity, projected from
  the git graph (React Flow + ELK for the graph view).
...
2. **G-provenance** asserted structurally: every rendered fact carries a resolvable claim id.
3. **Export-diff** green: snapshot == live render for the fixture (one render path proven).
4. **G-readonly + G-loopback** asserted: no store mutation via the server; token-gated; no egress on
   deterministic paths.
5. Golden projection transcripts reviewed (diffs, not silent drift).
6. Legacy `pnpm test:product` (1896) + core M1/M2 suites untouched.
