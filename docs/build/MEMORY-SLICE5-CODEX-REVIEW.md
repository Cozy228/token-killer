...
Authority docs (read first, do not relitigate their rulings): docs/build/MEMORY-SLICE5-GOAL-PROMPT.md (the 5 scope items + Explicitly OUT + invariants/acceptance), docs/build/MEMORY-SYNC-GOAL-PROMPT.md (hard invariants + acceptance bar), docs/build/MEMORY-DECISIONS.md (E3, E4, D27/D30), docs/build/MEMORY-SYNC-SETTLEMENTS.md (S8/S8a matrix; the `.ctx/push.jsonc` three-tier (b) and `.ctx/*.local.*` three-tier (c) zone rows), docs/build/MEMORY-SLICE5-NOTES.md (Decisions/Deviations/Open questions — the builder's rulings).
