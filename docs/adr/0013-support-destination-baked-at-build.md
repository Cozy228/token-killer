...
# `ctx support` destination is baked at build time, not configured at runtime

`ctx support` reaches **whoever packaged the build** — the maintainer. So the destination
...
- `__CTX_SUPPORT_EMAIL__` ← `process.env.CTX_SUPPORT_EMAIL` at build time
- `__CTX_SUPPORT_TEAMS__` ← `process.env.CTX_SUPPORT_TEAMS` (an Entra UPN)
- `__CTX_SUPPORT_GITHUB__` ← `process.env.CTX_SUPPORT_GITHUB` (`owner/repo` slug or a full
...
there is no `define`, so `resolveDestination` falls back to the matching `CTX_SUPPORT_*`
...
ADR 0011 routed support via runtime env vars (`CTX_SUPPORT_EMAIL` / `CTX_SUPPORT_TEAMS` /
`CTX_SUPPORT_GITHUB`) and shipped no baked destination, on the reasoning that each *deployment*
...
- **Bake at build via tsdown `define` (chosen).** The packager sets `CTX_SUPPORT_*` in the
...
- The `--email` / `--teams` / `--github` override flags are **removed**. `ctx support` takes
  only the channel positional + `--no-attach` / `--redact` / `-y`.
- A generic (non-enterprise) build bakes no destination ⇒ `ctx support` saves the report,
...
- To produce an enterprise build that reaches a helpdesk, set `CTX_SUPPORT_GITHUB` (or
