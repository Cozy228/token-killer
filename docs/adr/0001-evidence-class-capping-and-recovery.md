...
`ctx` inherited RTK's output filters wholesale, including RTK's **evidence-capping**:
...
`ctx`'s product stance is the opposite — retention-first, "0% savings is valid,
...
1. The safety gate (`base.ts`) detects omission by **regex-sniffing the output prose**
   for specific phrasings. `ctx`'s own handlers emit `... +N more` / `[+N more]` / bare
   `+N`, none of which match those patterns — so the gate is **blind to `ctx`'s own
   capping**. It only catches foreign/legacy phrasings no `ctx` handler produces.
...
5. **Declared, not sniffed.** A handler signals omission with a **structured field on
   ...
   regex is retired for `ctx`'s own handlers and kept only as a defense against foreign
...
6. **Recovery is always a snapshot read-back, never a re-run.** The inline pointer names
   the persisted snapshot **file path** (the exact bytes the digest came from). `ctx` never
   cites `ctx --raw <cmd>` as a recovery channel: a re-run re-executes the command, which
   can **drift** once the repo changes mid-turn and is **unsafe for mutating commands** —
   re-running `ctx --raw curl -X POST …` re-sends the request. This retroactively corrects
   the existing `ctx --raw` recovery hints in `curl` and `compactDiff`.
...
- **Keep RTK caps + RTK byte-parity, accept the carve-out** (make the gate recognize
  `ctx`'s own markers and whitelist "capping handlers"). Rejected: it institutionalizes
...
- **Several handlers now intentionally diverge from RTK byte-parity** and must be pulled
  ...
  - `curl` and `compactDiff` swap their `ctx --raw` re-run hint for a snapshot-file pointer.
...
- **Recovery-pointer fix (re-run → snapshot)**: `pytest` (`run with ctx --raw …` hint),
  `curl`, `compactDiff`.
