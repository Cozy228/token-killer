...
tk reports savings two ways, answering two different questions, both from **measured**
`raw − delivered` (never estimated):

- **Per-command** (`tk gain`, frozen/unchanged) — `saved / raw_tokens`, the compression
...
- **Per-session** (`tk gain --session`, this ADR) — of *everything the agent spent in a
  whole session* after tk onboarding, what fraction tk saved. Headline: **~27%** on
...
The per-command ratio cannot answer "of the whole session, how much did tk save?" — it
...
`saved` = **1.31M**, tk-touched `raw_tokens` = **1.97M**):
...
re-writes/re-reads. tk's `saved` is counted **once** (raw − compressed at command time),
...
tk's single interception point is **PreToolUse on shell commands**, so it only sees
**shell tool output** (`git` / `rg` / `ls` / test runners …) ≈ **40%** of unique content.
The remaining **~60% tk structurally cannot touch**:
...
- **built-in tool results** — `Read` / `Grep` / `Glob` / web-fetch / MCP outputs (NOT
  shell), which never pass through tk's shell hook;
...
`ANTHROPIC_BASE_URL`), which is out of scope here. Honest framing: *"tk saves ~27% of the
whole session by compressing the ~40% it can reach by two-thirds; the rest is non-shell
content tk's hook never sees."*
...
`(host, session_id)` — the first tk-wrapped command in that session. tk is never
...
- **footprint `input + cache_creation` summed per turn (the prior 8.56% headline).**
  ...
  content) and understating tk to 8.56%. Not apples-to-apples with tk's one-time `saved`.
...
- The ~60% tk cannot reach is exactly the surface the **request-body / L3 proxy** would
  unlock on Claude Code — recorded here as the ceiling, pursued elsewhere.

> Implementation status: the **basis decision is accepted**; the `tk gain --session` CLI
