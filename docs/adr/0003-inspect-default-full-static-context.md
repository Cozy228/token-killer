...
# Scope-aware `tg inspect`: user-level default, project opt-in, default `latest.json` write
...
core promise "no default repository scan, no default file writes". Today bare `tg inspect`
...
DESIGN insists there is **one** `tg inspect` that runs every analyzer and emits a unified
[Finding](../../CONTEXT.md#findings-and-optimization) report; `tg optimize context` is the
downstream consumer that reads the persisted report.

Two questions had to be settled. **(1) Default scope.** `tg inspect` must be runnable
...
1. **Static context is scope-aware, and the default scope is user-level.** Bare `tg inspect`
...
3. **inspect writes a per-scope `latest.json` on every run, by default**, into separate
   buckets — `~/.token-guard/user-context/inspect/latest.json` (no fingerprint) and
   `~/.token-guard/projects/<fingerprint>/inspect/latest.json`. This **supersedes** the
...
6. **No raw instruction bodies are persisted.** `latest.json` stores `file`, line range,
   `type`, counts, `scope`, and `body_hash`. `tg optimize context` re-reads the live file to
...
- `tg optimize context` is project-scoped by default and reads the project bucket; user-level
  work (`--surface skills`) reads the user bucket. When a bucket is absent it triggers a full
  inspect for that scope (`tg inspect --project` or `--user`). It re-reads project files and
