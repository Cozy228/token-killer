...
core promise "no default repository scan, no default file writes". Today bare `tk inspect`
...
DESIGN insists there is **one** `tk inspect` that runs every analyzer and emits a unified
[Finding](../../CONTEXT.md#findings-and-optimization) report; `tk optimize context` is the
downstream consumer that reads the persisted report.

Two questions had to be settled. **(1) Default scope.** `tk inspect` must be runnable
...
1. **Static context is scope-aware, and the default scope is user-level.** Bare `tk inspect`
...
3. **inspect writes a per-scope `latest.json` on every run, by default**, into separate
   buckets — `~/.token-killer/user-context/inspect/latest.json` (no fingerprint) and
   `~/.token-killer/projects/<fingerprint>/inspect/latest.json`. This **supersedes** the
...
6. **No raw instruction bodies are persisted.** `latest.json` stores `file`, line range,
   `type`, counts, `scope`, and `body_hash`. `tk optimize context` re-reads the live file to
...
- `tk optimize context` is project-scoped by default and reads the project bucket; user-level
  work (`--surface skills`) reads the user bucket. When a bucket is absent it triggers a full
  inspect for that scope (`tk inspect --project` or `--user`). It re-reads project files and
