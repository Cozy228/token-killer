# Code Graph for AI Coding Agents — research compendium (2026-06-18)

> **Scope.** A research-only survey (no design) of code-graph / repo-map / code-intelligence techniques,
> projects, and papers, compiled to inform a future token-killer (tk) feature. **Goal of the eventual
> feature:** give tk a code-graph capability that **cuts the tokens an agent spends (1) searching code and
> (2) reading files** — the two largest, most wasteful input-token sinks in agentic coding.
> **Method.** Cloned + read in full: codegraph, cocoindex-code. Read source / docs of: aider repo-map,
> Repomix, Probe, codesearch, CocoIndex, Continue, Roo Code, claude-context, Serena, SCIP. Surveyed 7
> papers (live arXiv fetch this pass). Companion to
> [`token-optimization-landscape-20260618.md`](./token-optimization-landscape-20260618.md).
> **This is a record of evidence and extracted technique — not a recommendation or a design.** Paper
> figures are as-extracted this pass; treat magnitudes as directional (see §10 Verification).

---

## 0. Table of contents

1. The "why" — token-economics evidence (where the tokens actually go)
2. Taxonomy — five families of code-context retrieval
3. Family A — Structural code graphs
4. Family B — Repo-map & context packing
5. Family C — Hybrid structural + lexical search
6. Family D — Embeddings / semantic retrieval
7. Family E — LSP & precomputed-index protocols
8. Research papers — methods, numbers, evaluation
9. Cross-cutting technique toolbox + compatibility with tk's invariants
10. Comparison matrices · Verification notes · References
...
|---|---|---|
| **Read/file-fetch operations dominate agent token spend** | SWE-Pruner (2601.16746) | **76.1% of tokens go to read operations** |
| **Repository *exploration* dominates tool-use turns** | FastContext (2606.14066) | **56.2% of tool-use turns are repo exploration** |
| Agentic coding vs chat token multiplier; input tokens dominate | "How Do AI Agents Spend Your Money?" (2604.22750) | **~1000× more tokens** than chat; **input-bound**; same task varies **up to 30×** |
...
| Accuracy is non-monotonic in spend | 2604.22750 | accuracy peaks at *intermediate* spend, then degrades — more reading ≠ better |

**Interpretation for the goal.** The two named targets — *code search* and *code reading* — are precisely the
two biggest documented sinks: ~56% of **turns** are exploration, and ~76% of **tokens** are reads. A
capability that lets the agent (a) locate the right code without a grep/glob/read loop and (b) receive the
*relevant* code (and only that) instead of whole files attacks the exact place the money is. Multiple
independent systems then confirm the savings are realizable (next sections): codegraph **47% fewer tokens /
58% fewer tool calls**; Codebase-Memory **10× fewer tokens**; FastContext **14–60% fewer tokens**.

A caution the same evidence carries: **context quality beats context volume** (SWE-ContextBench: a good
*summary* outperforms a full trajectory; bad context gives "limited or negative benefit"). So the win is not
"return more code faster" — it is "return the *right, ranked, minimal* code, honestly."
...
The families are not mutually exclusive (codegraph = A+FTS; Probe = C+LSP; claude-context = D+BM25). Sections
3–7 take each in turn; §9 extracts the reusable techniques across all of them.
...
### 3.1 codegraph (colbymchenry) — **MIT**, the reference implementation (cloned + read in full)
...
### 3.2 Codebase-Memory (arXiv 2603.27277) — the academic match
...
### 3.3 GitNexus — **PolyForm Noncommercial (ideas only, no code reuse)**
...
### 3.4 CodeGraphContext (Python) — pluggable backends
...
### 4.1 aider repo-map — the canonical ranked map (`aider/repomap.py`)
...
### 4.2 Repomix (TS, 26k★) — tree-sitter "code compression"
...
### 4.3 code2prompt (Rust) — template-based packaging; simplest packing; stable but slowing.
...
### 5.1 Probe (probelabs, Rust) — ripgrep + tree-sitter + BM25
...
Rust binary; **MCP primary** + npm SDK. **Zero setup, fully local.**

### 5.2 codesearch (flupkede, Rust) — hybrid vector + BM25 with RRF
...
Notably **Sourcegraph Cody and others retreated *away* from embeddings** at scale (§7.3). Embeddings are the
...
### 7.1 Serena — LSP-over-MCP, compiler-grade precision
...
### 7.2 SCIP — precomputed code-intelligence index format (Sourcegraph, successor to LSIF)
...
### 7.3 Sourcegraph Cody — historical (OSS repo private since Aug 2024; Free/Pro discontinued 2025)

Hybrid: local tree-sitter on open buffers + remote BM25 + (initially) **ada-002 embeddings**. **Pivoted away
from embeddings** toward sparse/keyword search at 100k-repo scale (privacy + scaling). Cloud-only; historical
architectural lesson: **embeddings are expensive and fragile at scale; hybrid keyword-first won out.**
...
version-aware index format a graph could **emit/consume** to interop with the wider ecosystem.

---

## 8. Research papers — methods, numbers, evaluation

### 8.1 Token economics & pruning (the "why" + "how much")

- **"How Do AI Agents Spend Your Money?" (2604.22750).** Measured 8 frontier models on SWE-Bench Verified.
  **~1000× tokens vs chat; input-token-bound; same task varies up to 30×; models can't predict their own
  spend (corr ≤0.39, systematically underestimate); accuracy non-monotonic in spend.** → establishes input/
  read is the lever and "more reading" can *hurt*.
- **SWE-Pruner (2601.16746).** A **0.6B "skimmer"** (Qwen3-Reranker backbone, CRF line-level labeling,
  ...
  **14.84× compression** single-turn; **18–26% fewer rounds**; **87.3% AST validity** (vs 0.29% naive
  ...
  the precision lever.
- **SWE-ContextBench (2602.08316).** 300 experience tasks + 99 derivatives; 5 settings (no-exp / free /
...
### 8.2 Retrieval & knowledge-graph methods

- **FastContext (Microsoft, 2606.14066).** **Repo exploration = 56.2% of tool-use turns.** A dedicated
...
- **Codebase-Memory (2603.27277).** See §3.2 — tree-sitter KG over MCP, **10× fewer tokens, sub-ms queries,
  83% quality**, 66 langs, 14 typed tools, community detection.
- **RepoCoder (2303.12570, EMNLP'23).** **Iterative retrieval-generation**: retrieve → generate partial →
...
- **CodeSearchNet Challenge (1909.09436).** 6 languages, 6M functions, 2M (code, doc) pairs, 99 expert NL
  queries w/ relevance grades; **rank-aware metric (NDCG)**. → the eval baseline for NL→code search; ranking
  quality, not binary recall, is what to measure.

### 8.3 What the papers collectively establish

1. **Where the tokens go:** input-bound; **~56% of turns = exploration**, **~76% of tokens = reads** — the
   exact two targets of the goal.
...
3. **How to evaluate:** measure **success-rate AND tokens together** (Pareto frontier); use **rank-aware
   ...
   rounds (FastContext/codegraph), not just tokens.

---

## 9. Cross-cutting technique toolbox
...
**incremental/provenance/honest-handback/MCP** machinery are fully compatible with tk's invariants. Family
**D (embeddings)** and the **RL-model** techniques are the ones that conflict (model/key/egress/training).
Family **E** splits: SCIP-as-interchange is a portable idea; LSP-as-core is too heavy but viable as a
complement.

---

## 10. Comparison matrices · Verification · References

### 10.1 Master comparison (all surveyed tools)
...
### 10.2 Approach trade-off (for the stated goal: cut search + read tokens, under tk's invariants)
...
- **Paper figures are as-extracted on 2026-06-18** via arXiv HTML/abstract. The 2026-dated IDs
  ...
  re-confirm against the primary PDF before quoting in external material. The two load-bearing numbers
  (read-ops ≈ 76%, exploration ≈ 56% of turns) are mutually corroborating and consistent with the
  qualitative "60–80% orientation" figure.
