...
>
> 下半部保留为 **Derivation & Evidence（推导与证据，含 216 条 `源:` 逐字引用与可抄代码）**，但其 **已按 Terminology Law 全程重组**（版本语言已清、capability-state 化、codemap/codeguide 命名统一），只把它当 source-cited 实现证据读，绑定口径以本契约为准。凡冲突，**契约 > 证据**。
...
**封闭组成**：Repository Overview / Symbol Inspector / Flow Inspector（bounded DAG，非完整 CFG/PDG/全仓画布）/ Domain Inspector（Observed/Derived/Inferred/Confirmed **分区**展示）/ Evidence Drawer（**仅对当前所选 fact** 按需展示 claims/policy/rationale/producer/freshness/coverage/decision）。
