> **[2026-07-04 P28] AMENDED OVERLAY — read together with repo-root `CTX-DESIGN.md` §9 (Contract Amendments Register).** D3 restructured (content types × carriers), D17 amended (3 tools: context/search/remember), D21③ outside scope, D33 amended (Node ≥22.5 flat, CLI = installer/bootstrap/fallback, P10/P28), the four-inspector codeguide composition superseded by CTX-DESIGN §6's page set, and the slice-DAG build order replaced by `CTX-IMPL.md` §9 M1–M5. Carried D-items remain authoritative HERE **permanently** — per P29 (reference-not-copy doc strategy) they are never restated into the ctx docs, only referenced; read-back map: `CTX-IMPL.md` §12.
