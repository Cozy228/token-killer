// `tk debug` entry (docs/debug-command-goal.md). Runs once on a tester's machine
// and writes ONE self-contained markdown bundle reviewing tk-on-this-machine, so a
// maintainer with only the bundle + the source can locate most problems. This is
// distinct from `tk inspect` (which audits the developer's OWN agent history/context,
// not tk itself), hence its own command rather than `inspect --debug`.
//
// Flags:
...
export async function runDebug(argv: string[], cwd: string = process.cwd()): Promise<number> {
  ...
  if (opts.error) {
    process.stderr.write(`tk debug: ${opts.error}\n`);
    return 1;
  }
  ...
  try {
    ...
    process.stderr.write(
      `tk debug: internal error: ${error instanceof Error ? error.message : String(error)}\n`,
    );
    return 3;
  }
}
