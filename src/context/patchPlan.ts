...
import { computeBodyMetrics } from "./metrics.js";
...
export function planForFinding(finding: ContextFinding, liveContent: string | undefined): PlanOutcome {
...
export function renderFrontmatterSetDiff(path: string, key: string, value: unknown, liveContent: string): string {
  const parsed = parseMarkdown(liveContent);
  ...
  if (parsed.frontmatter.present && !parsed.frontmatter.malformed) {
    ...
    return [`--- ${path}`, `+++ ${path}`, `@@ frontmatter @@`, ` (after line ${insertAfter})`, `+${line}`].join("\n");
  }
