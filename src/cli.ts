...
// Persist V8's compiled bytecode across invocations. Node 22.18 guarantees this
// API; when the wrapper exports NODE_COMPILE_CACHE it lands in ~/.contexa/v8-cache,
// otherwise Node uses its default cache location.
import module from "node:module";
try {
  module.enableCompileCache();
} catch {
  // Compile-cache failures must never affect command execution.
}
