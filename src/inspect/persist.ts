...
// Writes to the user-level `~/.contexa/advice/` with STABLE file names that
// overwrite prior files (no timestamped trend snapshots). Never the repo.

import { existsSync, mkdirSync, readFileSync, writeFileSync } from "node:fs";
import { join } from "node:path";

import { fingerprintSegment, contexaHome } from "../core/dataDir.js";
import type { Finding } from "./unified.js";

export function adviceDir(): string {
  return join(contexaHome(), "advice");
}
...
// never duplicated across projects or left stale. `ctx optimize context` reads
// the matching bucket.
...
export function userContextInspectDir(): string {
  return join(contexaHome(), "user-context", "inspect");
}
...
export function projectInspectDir(fingerprint: string): string {
  ...
  // project. `latest.json` is a derived artifact regenerated every `ctx inspect`, so
  // any pre-existing stripped-prefix dir is simply stale and harmless (no migration).
  return join(contexaHome(), "projects", fingerprintSegment(fingerprint), "inspect");
}
...
export function writeInspectBucket(bucket: ScopeBucket, report: InspectBucketReport): string {
  ...
  // Owner-only like every store under ~/.contexa/ (0700 dir / 0600 file). A
