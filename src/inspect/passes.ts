...
function probeCaches(
  ...
  const key: CacheKey | undefined = opts.scanCache || opts.habitsCache ? statKey(file) : undefined;
...
export function inspectSinglePass(
  ...
  return aggregate(discovery, opts, function* (files, group, tick) {
    for (const file of files) {
      ...
      const text = probe.scan && probe.habit ? undefined : readSourceText(file, opts.fileCache);
