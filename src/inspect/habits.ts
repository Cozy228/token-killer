...
export function summarizeHabits(sessions: Map<string, Session>): HabitStats {
  ...
  return {
    ...
    avg_tool_calls_per_session: active.length === 0 ? 0 : Math.round(totalToolCalls / active.length),
    max_tool_calls_in_session: toolCounts.length === 0 ? 0 : Math.max(...toolCounts),
    prompt_count: promptChars.length,
    avg_prompt_chars: promptChars.length === 0 ? 0 : Math.round(totalPromptChars / promptChars.length),
