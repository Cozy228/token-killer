// The one ctx diagnostics emitter, gated by `CTX_DEBUG` — one env var lights up the
// whole ctx runtime. Both the hook runtime AND the shim/compress path (src/cli.ts,
...
// `CTX_DEBUG=1` set, each step writes a structured `ctx debug: <scope> k=v …` line
...
// `$CONTEXA_HOME/debug.log` — `tail -f` it during a live session. This is a
...
export function tkDebugEnabled(): boolean {
  return Boolean(process.env.CTX_DEBUG);
}

// Default debug log location. Resolved per call so a test's CONTEXA_HOME
// override is always honored.
export function debugLogPath(): string {
  return join(contexaHome(), "debug.log");
}
...
// error stacks (errors.log) and are tailed into the `ctx support` bundle — the same
// data class #6 restricted. `mode:` applies only on creation, so chmod retroactively
// tightens a file left 0644 by a pre-fix ctx after an upgrade. Caller wraps in
...
// Emit one diagnostic line when CTX_DEBUG is set; no-op otherwise. `undefined`
...
export function tkDebug(scope: string, fields: Record<string, unknown> = {}): void {
  ...
  try {
    ...
    const body = `ctx debug: ${scope}${parts.length ? ` ${parts.join(" ")}` : ""}`;
...
// CONTEXA_HOME override is honored.
export function errorLogPath(): string {
  return join(contexaHome(), "errors.log");
}
...
// Record a FATAL ctx error UNCONDITIONALLY — unlike tkDebug, this is NOT gated on
// CTX_DEBUG. Rationale: when the host reports "PreToolUse hook errored", ctx exited
// non-zero and its stderr was swallowed by the host; the user had no breadcrumb and
// no reason to have set CTX_DEBUG beforehand. This is the crash-path writer — it
...
export function logFatalError(context: string, error: unknown): void {
  try {
    ...
    const line = `${timestamp()} ctx fatal: ${context}\n${detail}\n`;
    process.stderr.write(line);
    appendErrorLog(line);
  } catch {
    // a logging failure must never replace the original error
  }
  // A fatal crash is always ctx's OWN error — nudge the user toward `ctx support`.
  emitSupportHintOnce();
}

// One-shot stderr nudge toward `ctx support`, emitted only from ctx's OWN error sinks
...
const SUPPORT_HINT = "↳ Run `ctx support` to send this error + recent logs to the maintainer.";
...
// UNCONDITIONALLY (not gated on CTX_DEBUG), because a hook failure is observed only
...
// user cannot retroactively have set CTX_DEBUG. So the reason must always land in
...
export function recordHookError(
  ...
  try {
    ...
    const line = `${timestamp()} ctx hook-error: ${context}: ${detail}\n`;
    if (opts.surfaceStderr) process.stderr.write(line);
    appendErrorLog(line);
    // Only nudge toward `ctx support` when this error is being surfaced to the user
