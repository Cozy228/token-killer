...
// recursion guard in executor.ts strips TG_SHIM_DIR for tools tg *spawns*, but
// it cannot protect tg's own interpreter: if `node` were shimmed, a `tg <tool>`
...
// (see install.ts defaultTgExec), but this deny-set is the explicit, code-level
...
// tg-native verbs (read, smart, summary, err, test, deps, json, log, pipe)
