...
// recursion guard in executor.ts strips CTX_SHIM_DIR for tools ctx *spawns*, but
// it cannot protect ctx's own interpreter: if `node` were shimmed, a `ctx <tool>`
...
// ctx-native verbs (read, smart, summary, err, test, deps, json, log, pipe)
...
// True when `program` is a known dev tool ctx fronts (a wrapper exists for it).
// The passthrough-hardening guard (U2) uses this to decide whether a DIRECT
// `ctx <x>` (no CTX_SHIM_DIR) is allowed to run a real tool that has no specific
// handler but is still a tool ctx knows — e.g. `git --version`, a shimmable tool
