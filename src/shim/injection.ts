...
// commands with `ctx`. Coverage is probabilistic, so this is the fallback when
...
export function injectionBody(): string {
  return [
    "## Contexa",
    "",
    "Prefix shell commands with `ctx` to cut token usage on their output, e.g.",
    "`ctx git status`, `ctx npm test`, `ctx grep TODO src`. Contexa runs the real",
...
// A VS Code `.instructions.md` is a ctx-OWNED file (never a shared user doc), and
...
export function unwriteInjection(filePath: string): void {
  ...
  // If only ctx's block was in the file, ctx created it solely for that block —
...
// ~/.contexa/ file the caller surfaces with a printed note.
export function userInjectionPath(
  ...
  if (host === "vscode")
    return join(home, ".copilot", "instructions", "contexa-prefix.instructions.md");
  return join(contexaHome(), "copilot-instructions.md");
}

// Project-level target — the ONLY project-repo write, gated behind `ctx install
// --project`.
