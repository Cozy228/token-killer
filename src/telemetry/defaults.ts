...
// bake `false`; internal/enterprise builds set `CTX_TELEMETRY_DEFAULT=true` in the
// build env so a missing config.jsonc reads as opted-in. Users can still disable
// with `ctx telemetry disable` or by editing config.jsonc.
//
...
declare const __CTX_TELEMETRY_DEFAULT__: boolean | undefined;

export const TELEMETRY_DEFAULT_ENABLED: boolean =
  typeof __CTX_TELEMETRY_DEFAULT__ !== "undefined"
    ? __CTX_TELEMETRY_DEFAULT__
    : process.env.CTX_TELEMETRY_DEFAULT === "true";
