#!/usr/bin/env bash
#
# Remote ctx operations on CozyUltra Windows via ~/.ssh/config host `cozyultra`.
#
# Usage:
#   bash scripts/ssh-windows.sh              # interactive shell
#   bash scripts/ssh-windows.sh build        # pnpm install + build
#   bash scripts/ssh-windows.sh smoke        # quick ctx smoke
...
SSH_HOST="${CTX_SSH_HOST:-cozyultra}"
CTX_ROOT='C:\Users\cozy2\workspace\contexa'
...
case "$cmd" in
  shell | "")
    ...
    ;;
  build)
    remote "Set-Location '$CTX_ROOT'; pnpm install; pnpm build; node dist/cli.js --version"
    ;;
  smoke)
    remote "Set-Location '$CTX_ROOT'; node dist/cli.js hook check git status; node dist/cli.js --stats git status 2>&1 | Select-Object -Last 6"
    ;;
  setup-rg)
  ...
  clone-atlas)
    ...
    ;;
  dogfood-preview)
    remote "\$env:CTX_DOGFOOD_CWD='$ATLAS'; Set-Location '$CTX_ROOT'; pnpm exec pwsh -NoProfile -File scripts/windows-dogfood.ps1 -SkipLifecycle"
    ;;
  dogfood)
    remote "\$env:CTX_DOGFOOD_CWD='$ATLAS'; Set-Location '$CTX_ROOT'; pnpm test:windows-dogfood"
    ;;
  status)
    remote "Write-Host '=== ssh target ==='; hostname; whoami; Get-NetIPAddress -AddressFamily IPv4 | Where-Object IPAddress -notlike '127.*' | Select-Object InterfaceAlias, IPAddress; Write-Host '=== ctx ==='; Set-Location '$CTX_ROOT'; git branch --show-current; Test-Path dist/cli.js; Write-Host '=== atlas ==='; Test-Path '$ATLAS'; Write-Host '=== rg ==='; Get-Command rg -ErrorAction SilentlyContinue | Select-Object Source"
    ;;
  *)
