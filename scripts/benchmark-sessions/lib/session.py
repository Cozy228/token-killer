"""
Claude session execution for ctx (contexa) benchmark sessions.

Ported alongside rtk/scripts/benchmark-sessions/lib/runner.py and adapted to ctx
conventions: setup_rtk -> setup_tk, the per-session tracking artifact is ctx's
history JSONL under ~/.contexa (tk_db_path) rather than rtk's SQLite db.

Each ON VM runs Claude Code with the ctx command-rewrite hook installed; each OFF
VM runs the same prompt without ctx. Per-VM artifacts (stdout JSON, OTEL log, ctx
...
# ctx writes its tracking history under ~/.contexa (see src/core/dataDir.ts).
REMOTE_CONTEXA_HOME = "/home/ubuntu/.contexa"
...
class SessionResult:
    ...
    error: str | None = None
    # Local path to the pulled-back ctx tracking history (None when ctx was OFF).
...
async def setup_tk(name: str, setup_script: Path) -> None:
    """Install and enable ctx (and its Claude Code hook) on an ON VM."""
    remote_script = "/tmp/setup-ctx.sh"
...
def _build_session_command(task: TaskConfig, api_key: str, group: str) -> str:
    ...
    # On ON VMs the ctx hook is already installed (setup_tk); on OFF VMs it is not.
    return (
...
async def _run_session(
    ...
    try:
        ...
        # ON VMs accumulated ctx tracking history; pull it back too.
        if group == "on":
            tk_db_local = output_dir / f"{name}-tracking.jsonl"
            code, _, _ = await vm_exec(
                name,
                f"cat {REMOTE_CONTEXA_HOME}/projects/*/history.jsonl 2>/dev/null "
                f"> /tmp/ctx-tracking.jsonl",
            )
            if code == 0:
                await vm_pull(name, "/tmp/ctx-tracking.jsonl", tk_db_local)
