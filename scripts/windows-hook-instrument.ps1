...
# scripts\hooklog.js (logs invocation context + runs the REAL tk hook, captures its
# exit/stderr, passes stdout through, ALWAYS exits 0). After one `copilot -p` run, read
# C:\Users\cozy2\tk-hooklog.txt to see which field fires, how, and whether tk errors.
...
$logFile = Join-Path $home2 'tk-hooklog.txt'
$node    = 'C:\Program Files\nodejs\node.exe'
$hooklog = Join-Path $home2 'workspace\token-killer\scripts\hooklog.cjs'
...
if ($Restore) {
  ...
  Get-ChildItem $home2 -Filter 'tk-hl-*.cmd' -ErrorAction SilentlyContinue | Remove-Item -Force
  return
}
...
foreach ($f in @('command','bash','powershell')) {
  $cmdPath = Join-Path $home2 "tk-hl-$f.cmd"
...
$cfg.hooks.PreToolUse[0].command    = (Join-Path $home2 'tk-hl-command.cmd')
$cfg.hooks.preToolUse[0].bash       = (Join-Path $home2 'tk-hl-bash.cmd')
$cfg.hooks.preToolUse[0].powershell = (Join-Path $home2 'tk-hl-powershell.cmd')
